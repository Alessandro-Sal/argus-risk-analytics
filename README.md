...
## 🚀 XVA Counterparty Risk, Heston FFT Volatility, Black-Litterman, Basel III Liquidity, Structured Products & PRIIPs/SFDR (v9.16.0)

ARGUS v9.16.0 completa la suite istituzionale con 6 motori quantitativi di livello Tier-1 per banche d'investimento, desk derivati, tesorerie e gestori patrimoniali:
...
- **Nuovi Endpoint REST Headless v9.16.0 (`api/main.py`)**: `/api/v1/risk/xva`, `/api/v1/pricing/heston`, `/api/v1/optimize/black-litterman`, `/api/v1/risk/basel-liquidity`, `/api/v1/pricing/structured-products`, `/api/v1/regulatory/priips-sfdr`.
...
* **📦 Python Package Headless & CLI Console Scripts (`argus-risk`)**:
  ...
  # Esecuzione immediata via CLI console scripts
  argus-api      # Avvia il server REST FastAPI su http://127.0.0.1:8000
  argus-ui       # Avvia la Control Room Streamlit su http://127.0.0.1:8501
  ```
...
* **🏛️ Enterprise Quant Hardening, Arrow Binary Engine & Unified Rebalancing Architecture v9.6 (`core/rebalancing/`, `core/services/rebalancing_service.py`, `core/session_manager.py`, `api/main.py`, `core/cache_shield.py`, `core/risk_engine.py`)**:
  ...
  3. *Type-Safe Session State Governance (`ArgusSessionManager`)*: Gestore centralizzato e tipizzato per `st.session_state` con costanti univoche, default atomici, controlli di validazione (ISO 4217, profili MiFID II) e fallback trasparente per ambienti di test e microservizi.
  4. *High-Performance L2 Apache Arrow Feather Binary Caching (`CacheShield`)*: Serializzazione binaria LZ4/ZSTD ad altissima velocità per l'archivio SQLite della cache di mercato con abbattimento del 90% della latenza di I/O e compatibilità retroattiva per payload legacy.
  5. *Vettorizzazione 2D Merton Jump-Diffusion & Euler VaR Ledoit-Wolf Shrinkage*: Vettorizzazione 2D NumPy per la simulazione Monte Carlo con salti poissoniani ed eliminazione di 2.500.000 iterazioni pure Python; regolarizzazione spettrale PSD e shrinkage di Ledoit-Wolf per la decomposizione del rischio di Eulero e solutore vincolato Quadratic Programming SLSQP per Black-Litterman ($w_i \ge 0, \sum w_i = 1$).
  6. *Streamlit UI Fragment Isolation (`@st.fragment`)*: Isolamento granulare della toolbar di esportazione universale (`render_export_toolbar`), azzerando i re-run completi di pagina al download di file CSV/Excel su tutte le 50+ tabelle della piattaforma.
  7. *Desktop Packaging PyInstaller (`--build-exe`)*: Generazione automatica di eseguibili standalone Windows compilati a partire da `argus_desktop.spec`.
...
* **📥 Universal Financial Data Export Framework & Multi-Format Engine (`core/ui_export_utils.py`, `core/ui_utils.py`)**: Standardizzazione istituzionale dell'esportazione dati su tutte le tabelle della piattaforma (oltre 50 dataset nei 22 moduli operativi). Include download immediato in formato **CSV conforme agli standard europei/italiani** (codifica UTF-8 con Byte Order Mark `utf-8-sig` che preserva simboli di valuta `€` e caratteri accentati in Microsoft Excel) e **Excel professionale (.xlsx via openpyxl)** arricchito con palette *Dark Obsidian* (`#161B22`), accento ambra ARGUS (`#FF9900`), griglie contabili esplicite (`showGridLines = True`), auto-fit dinamico delle colonne, blocco pannelli (`freeze_panes = "A2"`) e formattazione numerica automatica (valute, percentuali, date e interi). Integrato con micro-popover a zero footprint verticale (`render_export_toolbar`), wrapper per card unificate (`render_table_with_export`) e caching binario `@st.cache_data` ad alte prestazioni.
