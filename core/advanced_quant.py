...
def compute_tail_copula_matrix(returns_df: pd.DataFrame, quantile_threshold: float = 0.05) -> dict:
    """
...
def compute_kelly_criterion_sizing(
    returns_df: pd.DataFrame, current_weights: Optional[dict] = None, risk_free_rate: float = None
) -> pd.DataFrame:
    """
...
def compute_interactive_trade_kelly(
    win_rate_pct: float, payoff_ratio: float, portfolio_capital_eur: float = 100000.0, stop_loss_pct: float = 5.0
) -> dict:
    """
    ...
    drawdown_risk = (
        "🟢 Basso (< 5%)"
        if half_kelly_pct < 15.0
        else ("🟡 Medio (5-15%)" if half_kelly_pct < 30.0 else "🔴 Elevato (> 15%)")
    )
