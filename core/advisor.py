...
def generate_quant_advisory_report(results: dict) -> dict:
        ...
        diagnostics.append(
            {
                ...
                "page_target": "pages/5_📋_Posizioni_e_Dettagli.py",
        ...
        diagnostics.append(
            {
                ...
                "page_target": "pages/3_🔴_Analisi_Rischio.py",
    ...
    if isinstance(opt, dict) and opt.get("max_sharpe"):
        opt_sharpe = (
            float(opt["max_sharpe"].get("sharpe") or 0.0) if opt["max_sharpe"].get("sharpe") is not None else 0.0
        )
            ...
            diagnostics.append(
                {
                    ...
                    "page_target": "pages/4_🔬_Modelli_Quantitativi.py",
    ...
    if not pos.empty and "trailing_pe" in pos.columns:
        ...
        if not expensive_stocks.empty:
            exp_tickers = ", ".join(
                [
                    f"**{r['ticker']}** (P/E: {float(r['trailing_pe']):.1f})"
                    for _, r in expensive_stocks.iterrows()
                    if pd.notna(r.get("trailing_pe"))
                ]
            )
            if exp_tickers:
                diagnostics.append(
                    {
                        ...
                        "page_target": "pages/6_🏛️_Valutazione_Aziendale.py",
        ...
        diagnostics.append(
            {
                ...
                "page_target": "pages/7_🌪️_Stress_Testing.py",
        ...
        diagnostics.append(
            {
                ...
                "description": "Il portafoglio ha una bassa sensibilità alle oscillazioni del mercato principale.",
