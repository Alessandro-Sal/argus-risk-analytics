...
def compute_brinson_attribution(results: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    sec_col = (
        "sector" if "sector" in active_pos.columns else ("gics_sector" if "gics_sector" in active_pos.columns else None)
    )
    if sec_col:
    ...
    if "unrealized_pnl_pct" not in active_pos.columns:
        if "unrealized_pnl" in active_pos.columns and "cost_basis" in active_pos.columns:
            active_pos["unrealized_pnl_pct"] = np.where(
                active_pos["cost_basis"] > 0, (active_pos["unrealized_pnl"] / active_pos["cost_basis"]) * 100.0, 0.0
            )
        else:
    ...
    p_sectors = (
        active_pos.groupby("sector")
        .agg(weight=("weight", "sum"), weighted_pnl=("weighted_pnl_pct", "sum"))
        .reset_index()
    )
    ...
    for sec in all_sectors:
        ...
        r_b = b_total_return + (
            1.5 if sec in ["Technology", "Communication Services"] else (-1.0 if sec == "Utilities" else 0.5)
        )
    ...
    return {"summary": summary, "attribution_df": df_attr}
...
def compute_carino_multi_period_attribution(subperiod_attributions: list[Dict[str, Any]]) -> Dict[str, Any]:
    """
    ...
    for t_idx, period in enumerate(subperiod_attributions):
        ...
        if isinstance(df_t, pd.DataFrame) and not df_t.empty:
            for _, row in df_t.iterrows():
                ...
                sector_effects[sec]["avg_portfolio_weight"] += float(row.get("weight_portfolio_pct", 0.0)) / len(
                    subperiod_attributions
                )
                sector_effects[sec]["avg_benchmark_weight"] += float(row.get("weight_benchmark_pct", 0.0)) / len(
                    subperiod_attributions
                )
