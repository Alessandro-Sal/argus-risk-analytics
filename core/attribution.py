...
def compute_brinson_attribution(
    ...
    if sec_col:
        active_pos["sector"] = active_pos[sec_col].fillna("Technology")
    else:
    ...
    all_sectors = sorted(list(set(list(BENCHMARK_SECTOR_WEIGHTS.keys()) + list(p_sector_map.keys()))))
    ...
    for sec in all_sectors:
        ...
        r_p = float(p_return_map.get(sec, b_total_return))
