...
def compute_cross_border_wealth_tax_comparison(
    ...
    for j_key, j in jur_data.items():
        ...
        effective_annual_rate_pct = (
            total_annual_tax / max(1.0, annual_capital_gain_eur + annual_foreign_income_eur)
        ) * 100.0
    ...
    it_ord_tax = next(
        (r["total_annual_tax_eur"] for r in comparison_rows if r["jurisdiction_key"] == "IT_ORDINARY"), 0.0
    )
