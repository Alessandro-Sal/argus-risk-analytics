    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            ...
            if any(sec in str(k).lower() for sec in ["password", "secret", "token", "key", "auth"]):
                ...
                fin in str(k).lower()
                for fin in ["saldo", "balance", "amount", "importo", "controvalore", "net_worth", "patrimonio"]
            ):
...
def setup_logging(
    ...
    sys_handler = RotatingFileHandler(
        str(SYSTEM_LOG_PATH), maxBytes=max_bytes, backupCount=backup_count, encoding="utf-8"
    )
    ...
    audit_handler = RotatingFileHandler(
        str(AUDIT_LOG_PATH), maxBytes=max_bytes, backupCount=backup_count, encoding="utf-8"
    )
    ...
    has_sys = any(
        isinstance(h, RotatingFileHandler) and "argus_system" in getattr(h, "baseFilename", "")
...
def measure_latency(metric_name: Optional[str] = None, logger_instance: Optional[logging.Logger] = None) -> Callable:
    """
...
def get_database_integrity_details() -> Dict[str, Any]:
    ...
    for db_file in ["argus_local.db", "yfinance_cache.db"]:
        ...
        try:
            ...
            db_report[db_file] = {"exists": True, "error": str(exc)}

    return db_report


def get_recent_logs(channel: str = "system", limit: int = 100, min_level: str = "INFO") -> List[Dict[str, Any]]:
    """
...
def generate_support_bundle(include_logs: bool = True, max_log_lines: int = 500) -> bytes:
    """
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("system_diagnostics.json", json.dumps(sanitize_dict(diag_serializable), indent=2, default=str))
        zf.writestr("environment_and_hardware.json", json.dumps(sanitize_dict(env_specs), indent=2, default=str))
        zf.writestr("database_status.json", json.dumps(sanitize_dict(db_status), indent=2, default=str))
...
def get_detailed_storage_and_memory_profile(results: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "integrity_all_ok": all("ok" in s.lower() or "integro" in s.lower() for s in integrity_statuses)
        if integrity_statuses
        else True,
...
def optimize_database_storage() -> Dict[str, Any]:
    ...
    return {"reclaimed_kb": round(reclaimed_bytes / 1024.0, 1), "details": ", ".join(results_list)}
...
def run_system_health_check(results: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        benchmarks.append(
            {
                "engine": "Hierarchical Risk Parity (HRP)",
                "latency_ms": round(hrp_lat, 2),
                "status": "🟢 Superato",
    ...
    try:
        ...
        benchmarks.append(
            {
                "engine": "Black-Scholes & Delta-Hedging",
                "latency_ms": round(bs_lat, 2),
                "status": "🟢 Superato",
                "category": "Derivati & Opzioni",
            }
        )
    except Exception as e:
        benchmarks.append(
    ...
    try:
        ...
        benchmarks.append(
            {
                "engine": "Market Regime Switching (3-State)",
                "latency_ms": round(reg_lat, 2),
                "status": "🟢 Superato",
                "category": "Macro Risk",
            }
        )
    ...
    try:
        ...
        benchmarks.append(
            {
                "engine": "Forensic Accounting (Beneish & Sloan)",
                "latency_ms": round(for_lat, 2),
                "status": "🟢 Superato",
                "category": "Contabilità Forense",
            }
        )
    except Exception as e:
        benchmarks.append(
    ...
    try:
        ...
        benchmarks.append(
            {
                "engine": "Technical Charting & Volume Profile",
                "latency_ms": round(ta_lat, 2),
                "status": "🟢 Superato",
    ...
    try:
        ...
        _ = compute_merton_jump_diffusion_simulation(
            sr_portfolio=sr_dummy, initial_value=100000.0, n_sims=100, time_horizon_days=60
        )
        mc_lat = (time.perf_counter() - t0) * 1000.0
        benchmarks.append(
            {
                "engine": "Merton Jump-Diffusion Simulation",
                "latency_ms": round(mc_lat, 2),
                "status": "🟢 Superato",
