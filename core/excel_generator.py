...
def generate_excel_in_memory(df: pd.DataFrame) -> io.BytesIO:
    ...
    f_kpi_lbl = workbook.add_format(
        {"bold": True, "font_size": 8, "font_color": "#64748B", "bg_color": "#F8FAFC", "border": 1, "align": "center"}
    )
    f_kpi_val = workbook.add_format(
        {
            "bold": True,
            "font_size": 11,
            "num_format": "€ #,##0.00",
    ...
    ws_data.conditional_format(
        start_row + 1, 7, last_row_data, 7, {"type": "data_bar", "bar_color": "#38BDF8", "bar_solid": True}
    )
    ws_data.autofit()
    ...
    ws_sim.write(
        "A2",
        "Inserisci una percentuale di shock (es. -10% o +5%) nelle celle evidenziate per ricalcolare il modello in tempo reale.",
        f_subtitle,
    )
    ...
    ws_sim.add_table(
        sim_start_row,
        0,
        sim_start_row + num_sim_rows,
        len(columns_spec_sim) - 1,
        {
            "name": "TableWhatIf",
            "data": sim_rows,
    ...
    ws_rw.write(
        "A2",
        "Riepilogo delle attività finanziarie estere con calcolo dell'imposta sul valore delle attività finanziarie all'estero (0.20%).",
        f_subtitle,
    )
    ...
    for _, row in df.iterrows():
        ...
        country_code = (
            "US"
            if not (tk.endswith(".MI") or tk.endswith(".DE") or tk.endswith(".PA"))
            else ("DE" if tk.endswith(".DE") else "IT")
        )
    ...
    ws_rw.add_table(
        rw_start_row,
        0,
        rw_start_row + num_rw_rows,
        len(columns_spec_rw) - 1,
        {
            "name": "TableQuadroRW",
            "data": rw_rows,
