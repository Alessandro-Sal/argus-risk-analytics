...
def generate_intraday_volume_profile(
    n_intervals: int = 16, open_rush_factor: float = 1.35, close_rush_factor: float = 1.55
) -> np.ndarray:
    """
...
def compute_twap_schedule(
    ...
    if df_ord.empty:
        return {
            ...
            "summary": {"total_orders": 0, "total_notional_eur": 0.0, "total_tranches": 0, "algo_type": "TWAP"},
        }
    ...
    for _, ord_row in df_ord.iterrows():
        ...
        vol_daily = float(
            ord_row.get(
                "volatility_daily",
                ord_row.get("vol_daily", (ord_row.get("volatility_ann_pct", 22.0) / 100.0) / np.sqrt(252.0)),
            )
        )
        ...
        for idx in range(n_intervals):
            ...
            est_exec_price = price * (
                1.0 + (est_slippage_bps / 10000.0) if "BUY" in action else 1.0 - (est_slippage_bps / 10000.0)
            )
...
def export_ibkr_basket_csv(orders: Union[pd.DataFrame, List[Dict[str, Any]]]) -> str:
    """
...
def export_directa_csv(orders: Union[pd.DataFrame, List[Dict[str, Any]]]) -> str:
    """
