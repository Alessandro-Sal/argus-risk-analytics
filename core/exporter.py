...
def _export_positions(results: dict, slug: str, ts: str) -> str:
    ...
    pos["pnl_status"] = pos["unrealized_pnl"].apply(lambda x: "profit" if x > 0 else ("loss" if x < 0 else "neutral"))
...
def _export_metrics(results: dict, slug: str, ts: str) -> str:
    ...
    row = {
        ...
        "risk_level": _risk_label(mk["volatility_annual_pct"], mk["var_95"]),
        # Ottimizzazione Markowitz
        "opt_max_sharpe_ratio": results.get("optimization", {}).get("max_sharpe", {}).get("sharpe"),
        "opt_max_sharpe_return": results.get("optimization", {}).get("max_sharpe", {}).get("return", 0) * 100
        if results.get("optimization")
        else None,
        "opt_max_sharpe_risk": results.get("optimization", {}).get("max_sharpe", {}).get("risk", 0) * 100
        if results.get("optimization")
        else None,
        "opt_min_vol_ratio": results.get("optimization", {}).get("min_vol", {}).get("sharpe"),
        "opt_min_vol_return": results.get("optimization", {}).get("min_vol", {}).get("return", 0) * 100
        if results.get("optimization")
        else None,
        "opt_min_vol_risk": results.get("optimization", {}).get("min_vol", {}).get("risk", 0) * 100
        if results.get("optimization")
        else None,
        # Qualità dati
