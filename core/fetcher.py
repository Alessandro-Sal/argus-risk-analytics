...
def get_engine(user: str, password: str, host: str,
               port: int = 3306, db: str = "investment_risk_bi"):
    """
    Restituisce un engine SQLAlchemy per MySQL. Se il database non esiste, lo crea.
    Inizializza anche le tabelle principali se mancanti.
    """
    import pymysql
    # Prima connessione senza specificare il db per crearlo
    ...
    # Seconda connessione al db specifico
    url = f"mysql+pymysql://{user}:{password}@{host}:{port}/{db}"
    engine = create_engine(url, echo=False)
    
    # Creazione delle tabelle base (se mancano) tramite SQLAlchemy Declarative Base
    from core.models import Base
    Base.metadata.create_all(engine)
    
    with engine.begin() as conn:
        # Migrazione per aggiungere colonne se la tabella assets era già stata creata precedentemente
        ...
        try:
            ...
            pass # Colonne già esistenti

        # Migrazione aggiuntiva per target_mean_price e peg_ratio
        try:
        ...
        # Migrazione per le nuove metriche BI (17 colonne)
        new_cols = [
...
def fetch_and_store(df_clean: pd.DataFrame,
    ...
    def download_ticker(t):
        try:
            ...
            hist = yf_ticker.history(start=t_start, end=end_date)
