...
def compute_piotroski_f_score(ticker: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if fin is not None and not fin.empty and bs is not None and not bs.empty:
            ...
            score += p1
            details.append({"Critero": "1. Utile Netto Positivo (ROA > 0)", "Valore": f"€ {ni_t/1e6:,.1f} M", "Esito": "🟢 Superato (+1)" if p1 else "🔴 Non Superato (0)"})
            ...
            score += p2
            details.append({"Critero": "2. Cash Flow Operativo Positivo", "Valore": f"€ {ocf_t/1e6:,.1f} M", "Esito": "🟢 Superato (+1)" if p2 else "🔴 Non Superato (0)"})
            ...
            score += p4
            details.append({"Critero": "4. Qualità degli Utili (OCF > Utile Netto)", "Valore": f"€ {ocf_t/1e6:,.1f}M vs € {ni_t/1e6:,.1f}M", "Esito": "🟢 Superato (+1)" if p4 else "🔴 Non Superato (0)"})
