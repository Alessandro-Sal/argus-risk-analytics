...
class ModularFactsheetBuilder:
    ...
    def build_pdf(  # noqa: C901
        ...
        buffer = io.BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=A4, leftMargin=32, rightMargin=32, topMargin=40, bottomMargin=42)
        ...
        tot_val = (
            ...
            else float(metrics.get("portfolio_value", 0.0) or 0.0)
        )
        ...
        if self.config.include_market_risk:
            story.append(
                Paragraph("1. MATRICE QUANTITATIVA DEL RISCHIO & VOLATILITÀ (SR 11-7)", styles["SectionTitle"])
            )
            risk_table = Table(
                [
                    [
        ...
        if has_page_2:
            story.append(
                Paragraph("3. STATO PATRIMONIALE CONSOLIDATO & TOTAL WEALTH BALANCE SHEET", styles["SectionTitle"])
            )
            ...
            if self.config.include_quadro_rw:
                story.append(
                    Paragraph(
                        "4. PROSPETTO FISCALE MONITORAGGIO ATTIVITÀ ESTERE (QUADRO RW / IVAFE)", styles["SectionTitle"]
                    )
                )
                rw_rows = [
                    [
            ...
            if self.config.include_fiscal_audit:
                story.append(
                    Paragraph("5. AUDIT FISCALE & COMPENSAZIONE MINUSVALENZE (TUIR ART. 67-68)", styles["SectionTitle"])
                )
                fisc_table = Table(
                    [
                        [
