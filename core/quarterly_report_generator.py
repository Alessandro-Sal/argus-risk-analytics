...
def generate_white_label_quarterly_pdf_report(
    ...
    from core.wealth.wealth_engine import compute_consolidated_net_worth, compute_total_wealth_brinson_attribution
    ...
    doc = SimpleDocTemplate(
        pdf_buffer, pagesize=A4, leftMargin=1.8 * cm, rightMargin=1.8 * cm, topMargin=1.8 * cm, bottomMargin=1.8 * cm
    )
    ...
    body_style = ParagraphStyle(
        "Body", parent=styles["Normal"], fontName="Helvetica", fontSize=9.5, leading=14, textColor=c_text
    )
    ...
    story.append(
        Paragraph(
            f"Cliente: <b>{client_name}</b> | Data di Riferimento: <b>{date.today().strftime('%d/%m/%Y')}</b>",
            meta_style,
        )
    )
    ...
    br_table_data = [
        [
            Paragraph("<b>Classe di Attivo</b>", meta_style),
            Paragraph("<b>Peso (%)</b>", meta_style),
            Paragraph("<b>Rendimento (%)</b>", meta_style),
            Paragraph("<b>Allocazione (%)</b>", meta_style),
            Paragraph("<b>Selezione (%)</b>", meta_style),
        ]
    ]
    ...
    # ── DISCLAIMER ──────────────────────────────────────────────
    story.append(
        HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#94a3b8"), spaceBefore=10, spaceAfter=8)
    )
    story.append(
        Paragraph(
