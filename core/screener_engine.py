...
MARKET_UNIVERSES: Dict[str, Dict[str, Any]] = {
    ...
    "🪙 Crypto Blue Chips & Digital Assets Ecosystem": {
        ...
        "tickers": ["BTC-USD", "ETH-USD", "SOL-USD", "COIN", "MSTR", "MARA", "RIOT", "SQ", "HOOD", "CLSK"],
    },
}
...
def _process_single_screener_ticker(tk: str, sr_bm_ret: pd.Series, force_refresh: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        currency = info.get("currency") or (
            "EUR" if tk.endswith("-EUR") or tk.endswith(".MI") or tk.endswith(".PA") or tk.endswith(".DE") else "USD"
        )
        ...
        if sector == "N/D":
            if quote_type == "CRYPTOCURRENCY" or "-" in tk or tk.startswith(("BTC", "ETH", "SOL")):
                ...
                or "ISHARES" in str(name).upper()
                or "VANGUARD" in str(name).upper()
            ):
                sector = "ETF / Fondi"
        ...
        if last_price > 0 and target_mean > 0:
            ...
            if ratio > 3.0 or ratio < 0.2:
                if tk.endswith(".CO") or (6.5 <= ratio <= 8.5):
                    target_mean /= 7.46
                elif tk.endswith(".ST") or (10.0 <= ratio <= 13.0):
                    target_mean /= 11.40
                elif tk.endswith(".OL"):
                    target_mean /= 11.50
                elif tk.endswith(".T") or ratio > 100:
                    target_mean /= 162.0
                elif tk.endswith(".L") and ratio > 50:
                    target_mean = (target_mean / 100.0) * 1.17
        ...
        if pd.notna(mcap_b):
            if mcap_b >= 200.0:
                mcap_cat = "Mega-Cap (>$200B)"
            elif mcap_b >= 10.0:
                mcap_cat = "Large-Cap ($10B-$200B)"
            elif mcap_b >= 2.0:
                mcap_cat = "Mid-Cap ($2B-$10B)"
            else:
                mcap_cat = "Small-Cap (<$2B)"
        else:
            mcap_cat = "N/D"
        ...
        if df_hist is not None and not df_hist.empty and len(df_hist) > 20:
            ...
            if not rets.empty:
                ...
                if len(closes) >= 50:
                    ...
                    if sma50 > 0:
                        p_sma50_pct = ((closes.iloc[-1] - sma50) / sma50) * 100.0
                if len(closes) >= 200:
                    sma200 = closes.rolling(200).mean().iloc[-1]
                    if sma200 > 0:
                        p_sma200_pct = ((closes.iloc[-1] - sma200) / sma200) * 100.0
        ...
        if pd.notna(debt_to_equity) and pd.notna(profit_margin):
            ...
            if pd.notna(roe) and roe > 15.0:
                z_score += 0.4

        piotroski_score = 6
        if pd.notna(roe) and roe > 10.0:
            piotroski_score += 1
        if pd.notna(fcf_yield) and fcf_yield > 4.0:
            piotroski_score += 1
        if pd.notna(debt_to_equity) and debt_to_equity < 0.8:
            piotroski_score += 1
        ...
        if pd.notna(upside_pct):
            score_val += np.clip(upside_pct * 1.5, -25, 25)
        if pd.notna(peg_ratio) and peg_ratio > 0:
            score_val += (1.5 - min(3.0, peg_ratio)) * 15.0

        score_qual = 50.0
        if pd.notna(roe):
            score_qual += np.clip((roe - 12.0) * 1.2, -20, 20)
        if pd.notna(z_score):
            score_qual += (z_score - 2.5) * 10.0

        score_risk = 50.0
        if pd.notna(vol_ann_pct):
            score_risk += (25.0 - min(45.0, vol_ann_pct)) * 1.5
        if pd.notna(sharpe_val):
            score_risk += np.clip(sharpe_val * 15.0, -20, 25)

        score_mom = 50.0
        if pd.notna(perf_1y_pct):
            score_mom += np.clip(perf_1y_pct * 0.8, -25, 25)
        if 45 <= rsi_val <= 65:
            score_mom += 10.0
        elif rsi_val > 75 or rsi_val < 30:
            score_mom -= 15.0

        argus_composite_score = np.clip(
            (score_val * 0.25) + (score_qual * 0.25) + (score_risk * 0.25) + (score_mom * 0.25), 5.0, 98.0
        )
...
def evaluate_custom_screener_query(df_screener: pd.DataFrame, query_str: str) -> tuple[pd.DataFrame, bool, str]:
    """
...
def simulate_pre_trade_impact(
    ...
    if current_positions_df is None or current_positions_df.empty:
        return {"valid": False, "message": "Nessun portafoglio attivo caricato per la simulazione."}
    ...
    if df_prices.empty or df_prices.shape[1] < 1 or clean_cand not in df_prices.columns:
        return {"valid": False, "message": f"Dati storici insufficienti per il titolo candidato {clean_cand}."}

    df_returns = df_prices.pct_change().dropna()
    if df_returns.empty or len(df_returns) < 30:
        return {"valid": False, "message": "Storico rendimenti troppo breve per una simulazione affidabile."}
...
def compute_optimal_candidate_weight(
    current_positions_df: pd.DataFrame, candidate_ticker: str, benchmark_ticker: str = "SPY"
) -> Dict[str, Any]:
    """
