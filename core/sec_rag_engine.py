...
def _calc_cosine_tfidf_score(
    q_tokens: List[str], doc_tokens: List[str], doc_freqs: Dict[str, int], total_docs: int
) -> float:
...
class LocalFilingVectorStore:
    ...
    def search(
        self, query: str, ticker_filter: Optional[str] = None, section_filter: Optional[str] = None, top_k: int = 4
        ...
        cosine_scores = []
        for c in candidates:
            s_cos = _calc_cosine_tfidf_score(q_tokens, c["_tokens"], self.doc_freqs, self.total_docs)
...
def chunk_financial_section(text: str, ticker: str, section_key: str, section_name: str) -> List[Dict[str, Any]]:
    """
...
def query_sec_filings_rag(
    ticker: str, query: str, section_filter: Optional[str] = None, top_k: int = 3
) -> Dict[str, Any]:
    """
    ...
    answer_lead = (
        f"Dall'analisi dei bilanci ufficiali depositati presso la SEC (Form 10-K/10-Q) per **{ticker.upper()}**:"
    )
