...
def init_settings_session_state(is_wealth_mode: bool = False) -> None:
    ...
    if "locale" not in st.session_state:
        st.session_state.locale = "it"
    if "accounting_notation" not in st.session_state:
        st.session_state.accounting_notation = "standard"
    if "data_environment" not in st.session_state:
        st.session_state.data_environment = "Auto (Ibrido)"
    if "offline_mode" not in st.session_state:
        st.session_state.offline_mode = False
    if "ui_theme" not in st.session_state:
        st.session_state.ui_theme = "Midnight Obsidian"

    # DB Connection & Sync Defaults
    if "db_host" not in st.session_state:
        st.session_state.db_host = os.getenv("STREAMLIT_DB_HOST", "localhost")
    if "db_port" not in st.session_state:
        st.session_state.db_port = _detect_default_port(st.session_state.db_host, 3306)
    if "db_user" not in st.session_state:
        st.session_state.db_user = os.getenv("STREAMLIT_DB_USER", "root")
    if "db_pass" not in st.session_state:
        st.session_state.db_pass = os.getenv("STREAMLIT_DB_PASS", "root")
    ...
    if "risk_estimation_method" not in st.session_state:
        st.session_state.risk_estimation_method = "Parametrico (Cornish-Fisher)"
    if "risk_lookback_period" not in st.session_state:
        st.session_state.risk_lookback_period = "Ultimo Anno (252g)"
    if "risk_decay_factor" not in st.session_state:
        st.session_state.risk_decay_factor = 0.94
    if "benchmark" not in st.session_state:
        st.session_state.benchmark = "SPY"
    if "rf_mode" not in st.session_state:
        st.session_state.rf_mode = "Auto (Live Market)"
    if "custom_rf_rate_pct" not in st.session_state:
        st.session_state.custom_rf_rate_pct = 2.75

    # ── Parametri Specifici Wealth Management ─────────────────────
    if "wealth_planning_horizon_years" not in st.session_state:
        st.session_state.wealth_planning_horizon_years = 25
    if "wealth_expected_return_pct" not in st.session_state:
        st.session_state.wealth_expected_return_pct = 6.50
    if "wealth_inflation_rate_pct" not in st.session_state:
        st.session_state.wealth_inflation_rate_pct = 2.00
    if "wealth_tax_regime" not in st.session_state:
        st.session_state.wealth_tax_regime = "Ordinario (26%)"
    if "wealth_stress_scenario" not in st.session_state:
        st.session_state.wealth_stress_scenario = "Base (Nessuno Shock)"
    if "wealth_budget_preset" not in st.session_state:
        st.session_state.wealth_budget_preset = "50/30/20 Standard"
    if "wealth_budget_needs_pct" not in st.session_state:
        st.session_state.wealth_budget_needs_pct = 50.0
    if "wealth_budget_wants_pct" not in st.session_state:
        st.session_state.wealth_budget_wants_pct = 30.0
    if "wealth_budget_savings_pct" not in st.session_state:
        st.session_state.wealth_budget_savings_pct = 20.0
    if "wealth_fire_swr" not in st.session_state:
        st.session_state.wealth_fire_swr = 4.0
    if "wealth_target_retirement_age" not in st.session_state:
        st.session_state.wealth_target_retirement_age = 67
    if "wealth_pension_deduction_limit" not in st.session_state:
        st.session_state.wealth_pension_deduction_limit = 5164.57
    if "portfolio_name" not in st.session_state:
        st.session_state.portfolio_name = "Master Wealth" if is_wealth_mode else "Portafoglio Principale"
...
def render_settings_dialog(current_module: str = "risk") -> None:
    ...
    with tab_mod:
        if not is_wealth:
            ...
            with col_b1:
                ...
                b_idx = (
                    bench_options.index(current_bench)
                    if current_bench in bench_options[:-1]
                    else bench_options.index("Custom...")
                )
                ...
                if sel_b == "Custom...":
                    cust_b = (
                        st.text_input(
                            "Ticker Custom",
                            value="" if current_bench in bench_options[:-1] else current_bench,
                            key="sb_custom_bench",
                        )
                        .strip()
                        .upper()
            ...
            if st.session_state.rf_mode == "Manuale":
                ...
                st.caption(
                    f"🏛️ Tasso Attivo: **{active_rf_info.get('rate_pct', 2.75):.2f}%** ({active_rf_info.get('currency', 'EUR')} - {'Live Yield Curve' if active_rf_info.get('is_live') else 'BCE/Fed reference'})"
                )
            ...
            with col_w1:
                ...
                sel_horizon = st.slider(
                    "Orizzonte di Pianificazione (Anni)",
                    min_value=5,
                    max_value=50,
                    value=curr_horizon,
                    step=1,
                    key="sb_wealth_horizon",
                )
            ...
            with col_w2:
                ...
                sel_exp_ret = st.slider(
                    "Rendimento Nominale Atteso (%)",
                    min_value=0.0,
                    max_value=15.0,
                    value=curr_exp_ret,
                    step=0.25,
                    key="sb_wealth_exp_return",
                )
            ...
            with col_w5:
                tax_regimes = [
                    "Ordinario (26%)",
                    "Riforma Unificata 2026 (26%)",
                    "Agevolato Titoli Stato (12.5%)",
                    "Dichiarativo / Quadro RW",
                ]
                t_idx = (
                    tax_regimes.index(st.session_state.get("wealth_tax_regime", "Ordinario (26%)"))
                    if st.session_state.get("wealth_tax_regime") in tax_regimes
                    else 0
                )
                sel_tax = st.selectbox(
                    "Regime Fiscale di Riferimento", tax_regimes, index=t_idx, key="sb_wealth_tax_regime"
                )
            ...
            with col_w6:
                ...
                sel_stress = st.selectbox(
                    "Scenario di Stress Predefinito", stress_scenarios, index=s_idx, key="sb_wealth_stress"
                )
                st.session_state.wealth_stress_scenario = sel_stress

            # Modello di Budgeting & Regola FIRE
            st.markdown(
                "<hr style='margin: 10px 0 8px; border: none; border-top: 1px solid rgba(255,255,255,0.06);'>",
                unsafe_allow_html=True,
            )
            preset_options = [
                "50/30/20 Standard",
            ...
            with col_n:
                n_val = st.number_input(
                    ...
                    min_value=5.0,
                    max_value=90.0,
                    value=float(st.session_state.get("wealth_budget_needs_pct", 50.0)),
                    step=5.0,
                    format="%.0f",
                    key="sb_wb_needs",
                )
            ...
            with col_w:
                w_val = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=90.0,
                    value=float(st.session_state.get("wealth_budget_wants_pct", 30.0)),
                    step=5.0,
                    format="%.0f",
                    key="sb_wb_wants",
                )
            ...
            with col_s:
                s_val = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=90.0,
                    value=float(st.session_state.get("wealth_budget_savings_pct", 20.0)),
                    step=5.0,
                    format="%.0f",
                    key="sb_wb_savings",
                )
            ...
            with col_swr:
                swr_val = st.number_input(
                    "SWR FIRE % (Safe Withdrawal Rate)",
                    min_value=1.5,
                    max_value=8.0,
                    value=float(st.session_state.get("wealth_fire_swr", 4.0)),
                    step=0.1,
                    format="%.1f",
                    key="sb_wealth_swr_input",
                    help="Tasso di prelievo annuo sostenibile per il calcolo del target FIRE.",
                )
            ...
            with col_age:
                age_val = st.number_input(
                    ...
                    min_value=30,
                    max_value=75,
                    value=int(st.session_state.get("wealth_target_retirement_age", 67)),
                    step=1,
                    key="sb_wealth_age_input",
                    help="Età anagrafica prevista per l'uscita dal lavoro o rendita da capitale.",
                )
    ...
    with tab_conn:
        ...
        off_hlp = (
            "Usa il database locale embedded SQLite senza dipendere da MySQL."
            if is_wealth
            ...
            with st.expander("🔌 Connessione & Schema Database", expanded=False):
                ...
                with col_pw:
                    st.session_state.db_pass = st.text_input(
                        "Password", type="password", value=st.session_state.db_pass, key="sb_db_pass"
                    )
                ...
                if is_wealth:
                    if st.button(
                        "📥 Allinea DB Locale SQLite",
                        key="sb_btn_sync_sqlite",
                        use_container_width=True,
                        help="Copia tutti i dati e movimenti da MySQL al database locale SQLite per lavorare offline.",
                    ):
                        try:
                            ...
                            st.success(
                                f"Allineati {sync_res.get('wealth_cashflow', 0)} movimenti e {sync_res.get('wealth_accounts', 0)} conti in SQLite!"
                            )
                        except Exception as ex:
                            st.error(f"Errore sincronizzazione: {ex}")

    st.markdown(
        "<hr style='margin: 12px 0 8px; border: none; border-top: 1px solid rgba(255,255,255,0.08);'>",
        unsafe_allow_html=True,
    )
...
def _execute_full_session_reset(is_wealth_mode: bool = False) -> None:
    ...
    in_wealth = is_wealth_mode or any(
        w in cur_p for w in ["12_", "13_", "14_", "15_", "16_", "17_", "18_", "19_", "20_", "21_"]
    )
...
def render_settings_sidebar(current_module: str = "risk") -> None:
    ...
    if st.button(
        "⚙️ Impostazioni Globali & Parametri",
        key="sb_btn_open_settings_dialog",
        use_container_width=True,
        help="Apri le impostazioni di sistema, preferenze e parametri dei motori in un modale centralizzato.",
    ):
    ...
    with col_btn1:
        if st.button(
            "🧹 Svuota Cache",
            key="sb_btn_clean_cache_light",
            use_container_width=True,
            help="Invalida la cache di calcolo e i prezzi di mercato in memoria senza perdere il portafoglio caricato.",
        ):
            st.cache_data.clear()
            st.cache_resource.clear()
            try:
    ...
    with col_btn2:
        if st.button(
            "♻️ Reset Sessione",
            key="sb_btn_reset_session_compact",
            use_container_width=True,
            help="Elimina tutti i risultati di calcolo, la cache di mercato e i dati caricati, mantenendo le configurazioni di sistema e database.",
        ):
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        in_wealth_page = any(
            w in cur_page_name for w in ["12_", "13_", "14_", "15_", "16_", "17_", "18_", "19_", "20_", "21_"]
        )
        if "argus_portal_mode" not in st.session_state:
            ...
            not in_wealth_page
            and any(
                w in cur_page_name for w in ["0_", "1_", "2_", "3_", "4_", "5_", "6_", "7_", "8_", "9_", "10_", "11_"]
            )
            and st.session_state.argus_portal_mode != "📊 Risk Analytics"
        ):
            st.session_state.argus_portal_mode = "📊 Risk Analytics"
        ...
        eye_sidebar_svg = get_argus_eye_svg(
            size=32, animated=True, accent=accent, unique_id=f"sb_brand_eye_{'wealth' if is_wealth_mode else 'risk'}"
        )
        ...
        if "sb_accounting_select" in st.session_state:
            st.session_state.accounting_notation = (
                "parentheses" if "Wall Street" in st.session_state.sb_accounting_select else "standard"
            )
        ...
        custom_rf_dec = (
            (st.session_state.custom_rf_rate_pct / 100.0) if st.session_state.rf_mode != "Auto (Live Market)" else None
        )
        active_rf_info = get_active_risk_free_rate(
            currency=st.session_state.base_currency, custom_override=custom_rf_dec
        )
        ...
        curr_active_db = st.session_state.get(
            "wealth_db_name" if is_wealth_mode else "risk_db_name", st.session_state.get("db_name")
        )
        ...
        port_text_style = (
            "color:#ffffff; font-weight:700;" if has_port else "color:#e3b341; font-style:italic; font-weight:600;"
        )
        ...
        if is_wealth_mode:
            ...
            w_status_badge = (
                ...
                if is_offline
                else '<span style="font-size:10px; font-weight:800; color:#34d399; background:rgba(16, 185, 129, 0.20); padding: 2px 7px; border-radius:12px; letter-spacing:0.5px;">🟢 LIVE DB</span>'
            )
            ...
            try:
                ...
                if not df_wports.empty and len(df_wports) >= 1:
                    col_id = (
                        "portfolio_id"
                        if "portfolio_id" in df_wports.columns
                        else ("id" if "id" in df_wports.columns else df_wports.columns[0])
                    )
                    ...
                    st.selectbox(
                        ...
                        format_func=lambda pid: (
                            "-- Seleziona Profilo --" if pid is None else f"📁 {w_names.get(pid, f'Profilo #{pid}')}"
                        ),
                        index=w_idx,
        ...
        st.markdown(
            '<div class="sidebar-section-header" style="margin-top: 10px; margin-bottom: 4px; font-size: 10px; font-weight: 800; color: #8b949e; text-transform: uppercase; letter-spacing: 0.8px;">CAMBIA AMBIENTE</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            '<div class="sidebar-section-header" style="margin-top: 10px; margin-bottom: 8px; font-size: 10px; font-weight: 800; color: #8b949e; text-transform: uppercase; letter-spacing: 0.8px;">NAVIGAZIONE WORKSPACE</div>',
            unsafe_allow_html=True,
        )
        ...
        def _is_mod_active(mod_dict: dict, cur_page_str: str) -> bool:
            ...
            if "0controlroom" in mod_c or "0controlroom" in key_c:
                return ("0controlroom" in cur_c) or (
                    "pages" not in cur_page_str.lower() and (cur_c == "" or "controlroom" in cur_c or "app" in cur_c)
                )
        ...
        for mod in active_nav_modules:
            ...
            if not mod["has_subtabs"]:
                ...
                if st.button(btn_label, key=f"nav_direct_{mod['key']}", use_container_width=True, type=btn_type):
                ...
                with st.expander(expander_label, expanded=is_active):
                    ...
                    for idx, sub in enumerate(mod["subtabs"]):
                        ...
                        is_sub_active = is_active and (
                            current_active_target == sub["target"] or (not current_active_target and idx == 0)
                        )
        ...
        <div style="text-align: center; padding: 10px 0 2px; border-top: 1px solid rgba(255,255,255,0.06); margin-top: 10px;">
            ...
            <div style="font-size: 10px; font-weight: 600; color: #ff9900; margin-top: 2px;">v9.18.0 Institutional Ecosystem</div>
        </div>
        """,
            unsafe_allow_html=True,
        )
