...
def compute_technical_indicators(df_prices: pd.DataFrame) -> Dict[str, Any]:
    ...
    is_squeeze = (
        float(bb_bandwidth.iloc[-1]) <= float(bandwidth_recent.quantile(0.20)) if len(bandwidth_recent) > 10 else False
    )
    ...
    pos_di = 100.0 * (
        pd.Series(pos_dm, index=df.index).ewm(alpha=1 / 14, adjust=False).mean() * 14.0 / (tr_smooth + 1e-9)
    )
    neg_di = 100.0 * (
        pd.Series(neg_dm, index=df.index).ewm(alpha=1 / 14, adjust=False).mean() * 14.0 / (tr_smooth + 1e-9)
    )
...
def compute_volume_profile(df_prices: pd.DataFrame, n_bins: int = 20) -> Dict[str, Any]:
    ...
    volume = (
        df_prices["volume"].astype(float) if "volume" in df_prices.columns else pd.Series(1.0, index=df_prices.index)
    )
...
def compute_technical_confluence_score(df_prices: pd.DataFrame) -> Dict[str, Any]:
    ...
    if not indicators:
        return {"score": 50.0, "verdict": "Neutro", "verdict_icon": "🟡", "factors": []}
    ...
    if indicators["last_ema20"] > indicators["last_ema50"]:
        score += 12.0
        factors.append(
            {"indicator": "EMA 20 vs EMA 50", "status": "🟢 Rialzista", "impact": "+12 pts", "note": "EMA 20 > EMA 50"}
        )
    else:
        score -= 12.0
        factors.append(
            {"indicator": "EMA 20 vs EMA 50", "status": "🔴 Ribassista", "impact": "-12 pts", "note": "EMA 20 < EMA 50"}
        )
    ...
    if indicators["last_close"] > indicators["last_sma200"]:
        ...
        factors.append(
            {
                "indicator": "Prezzo vs SMA 200",
                "status": "🟢 Trend Primario Toro",
                "impact": "+13 pts",
                "note": "Prezzo sopra la media a 200gg",
            }
        )
    else:
        score -= 13.0
        factors.append(
            {
                "indicator": "Prezzo vs SMA 200",
                "status": "🔴 Trend Primario Orso",
                "impact": "-13 pts",
                "note": "Prezzo sotto la media a 200gg",
            }
        )
    ...
    if indicators["last_macd_hist"] > 0:
        ...
        factors.append(
            {
                ...
                "status": "🟢 Momentum Positivo",
                "impact": "+10 pts",
                "note": "Istogramma MACD sopra lo zero",
            }
        )
    else:
        score -= 10.0
        factors.append(
            {
                "indicator": "MACD Histogram",
                "status": "🔴 Momentum Negativo",
                "impact": "-10 pts",
                "note": "Istogramma MACD sotto lo zero",
            }
        )
    ...
    if 45 <= rsi <= 65:
        ...
        factors.append(
            {
                "indicator": "RSI (14)",
                "status": "🟢 Zona Costruttiva",
                "impact": "+5 pts",
                "note": f"RSI a {rsi:.1f} in zona neutro-positiva",
            }
        )
    elif rsi > 70:
        score -= 8.0
        ...
        factors.append(
            {
                "indicator": "RSI (14)",
                "status": "🟢 Ipervenduto (Opportunità)",
                "impact": "+8 pts",
                "note": f"RSI a {rsi:.1f} sotto 30",
            }
        )
    else:
        ...
        factors.append(
            {
                ...
                "status": "🟢 Trend Solido",
                "impact": "+5 pts",
                "note": f"ADX a {indicators['adx_latest']:.1f} (> 25)",
            }
        )
    else:
        factors.append(
        ...
        factors.append(
            {
                ...
                "status": "⚡ Imminente Volatilità",
                "impact": "Alert",
                "note": "Bande di Bollinger molto strette",
            }
        )
    ...
    return {"score": float(final_score), "verdict": verdict, "verdict_icon": verdict_icon, "factors": factors}
...
def compute_multi_timeframe_analysis(df_prices: pd.DataFrame) -> Dict[str, Any]:
    ...
    df_w = df_prices.resample("W").agg({"close": "last", "open": "first", "high": "max", "low": "min"}).dropna()
    ...
    is_aligned = (trend_d.startswith("🟢") and trend_w.startswith("🟢")) or (
        trend_d.startswith("🔴") and trend_w.startswith("🔴")
    )
    ...
    return {
        ...
        "alignment_text": "🟢 Trend Allineato (Confluenza Multi-Timeframe)"
        if is_aligned
        else "🟡 Trend Discordante tra Daily e Weekly",
    }
