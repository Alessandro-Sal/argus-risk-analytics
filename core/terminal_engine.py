...
def _clean_box_line(s: str) -> str:
    ...
    cleaned = []
    for ch in s:
        if ord(ch) >= 0x1F300 or (
            0x2600 <= ord(ch) <= 0x27BF and ch not in ("▲", "▼", "●", "─", "┌", "┐", "└", "┘", "├", "┤", "│")
        ):
            cleaned.append(" ")
        else:
            cleaned.append(ch)
...
try:
    from core.streaming_engine import MarketTick, TickRingBuffer, generate_mock_streaming_ticks
...
def fetch_live_ticker_quote(
    ticker: str, force_refresh: bool = False, fallback_price: Optional[float] = None
) -> Dict[str, Any]:
    """
    ...
    default_p = (
        ...
        if (fallback_price is not None and fallback_price > 0)
            ...
            ArgusTerminalEngine.DEFAULT_PRICES.get(sym, 150.0)
            if hasattr(ArgusTerminalEngine, "DEFAULT_PRICES")
            else 150.0
        )
    )
    fallback_res = {
...
def evaluate_pre_trade_risk(
    ...
    if current_day_pnl_eur <= -abs(lim.max_daily_loss_eur) and side.upper() == "BUY":
        ...
        status = "BLOCKED"
        reasons.append(
            f"CIRCUIT BREAKER ATTIVO: PnL Day (€ {current_day_pnl_eur:,.2f}) ha violato il limite max di perdita giornaliera (€ -{lim.max_daily_loss_eur:,.2f}). Ordini BUY bloccati."
        )
    ...
    if order_notional > lim.max_order_notional_eur:
        ...
        status = "BLOCKED"
        reasons.append(
            f"LIMITE NOZIONALE SUPERATO: L'ordine (€ {order_notional:,.2f}) supera il tetto massimo consentito (€ {lim.max_order_notional_eur:,.2f})."
        )
    ...
    post_port_notional = max(
        1.0, current_portfolio_notional + (order_notional if side.upper() == "BUY" else -order_notional)
    )
    ...
    if post_weight > lim.max_single_asset_weight and side.upper() == "BUY":
        ...
        reasons.append(
            f"ALLERTA CONCENTRAZIONE: Il peso post-trade di {sym} ({post_weight * 100:.1f}%) supera la soglia raccomandata del {lim.max_single_asset_weight * 100:.0f}%."
        )
...
def compute_pnl_attribution(df_positions: pd.DataFrame, all_quotes: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    """
    ...
    qty_col = (
        "qty_net" if "qty_net" in active_pos.columns else ("quantity" if "quantity" in active_pos.columns else "shares")
    )
...
def fetch_market_catalysts(tickers: List[str]) -> List[Dict[str, Any]]:
    ...
    sample_events = [
        (
            "AAPL",
            "EARNINGS",
            "Q3 Earnings Conference Call & Services Revenue Guidance",
            "HIGH 🔴",
            "BULLISH 🟢",
            "Domani post-close",
        ),
        ...
        (
            "TSLA",
            "REGULATORY",
            "Full Self-Driving (FSD) European Regulatory Approval Status",
            "HIGH 🔴",
            "VOLATILE ⚡",
            "02 Set",
        ),
        ...
        (
            "BTC-USD",
            "FLOWS",
            "Institutional Spot ETF Net Inflow Surge (+$320M)",
            "MEDIUM 🟡",
            "BULLISH 🟢",
            "Live Stream",
        ),
...
class ArgusTerminalEngine:
    ...
    def get_or_create_ring_buffer(
        self, ticker: str = "AAPL", initial_price: Optional[float] = None, capacity: int = 500
    ) -> Any:
            ...
            if ticker_clean not in self._ring_buffers or self._ring_buffers[ticker_clean] is None:
                if TickRingBuffer:
                    ...
                    start_px = (
                        ...
                        else self.DEFAULT_PRICES.get(ticker_clean, 150.0)
                    )
    ...
    def place_order(
        ...
        if isinstance(df_pos, pd.DataFrame) and not df_pos.empty and "ticker" in df_pos.columns:
            ...
            if not m.empty:
                px_col = (
                    "current_price"
                    if "current_price" in m.columns
                    else ("last_price" if "last_price" in m.columns else ("wacp" if "wacp" in m.columns else None))
                )
                if px_col:
                    mkt_px = float(m[px_col].iloc[0] or 150.0)
                val_col = (
                    "current_value"
                    if "current_value" in m.columns
                    else ("market_value" if "market_value" in m.columns else None)
                )
                if val_col:
        ...
        return (
            True,
            f"✅ Ordine {ord_id} inviato con successo: {sd} {q:,.1f} {sym} @ {order_type} (${fill_px:.2f}){warn_msg}",
            new_ord,
        )
    ...
    def execute_command(self, command_str: str, context: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        ...
        if isinstance(active_pos, pd.DataFrame) and not active_pos.empty and "ticker" in active_pos.columns:
            val_col = (
                ...
                else ("market_value" if "market_value" in active_pos.columns else None)
            )
            if val_col:
        ...
        if first_token in ("WL", "WATCHLIST", "MONITOR", "TAPE"):
            if len(tokens) >= 3 and tokens[1].upper() in ("ADD", "+"):
                ...
                return TerminalCommandResult(
                    command=raw_cmd, status="SUCCESS", output_text=f"Aggiunto ticker '{sym_add}' alla Watchlist attiva."
                )
            if len(tokens) >= 3 and tokens[1].upper() in ("DEL", "DELETE", "REMOVE", "-"):
                ...
                return TerminalCommandResult(
                    command=raw_cmd, status="SUCCESS", output_text=f"Rimosso ticker '{sym_del}' dalla Watchlist attiva."
                )
            if len(tokens) == 2 and tokens[1].upper() in ("RESET", "CLEAR"):
                self.custom_watchlist = ["SPY", "QQQ", "NVDA", "AAPL", "MSFT", "BTC-USD", "GC=F", "EURUSD=X"]
                return TerminalCommandResult(
                    command=raw_cmd, status="INFO", output_text="Watchlist ripristinata ai benchmark globali standard."
                )
        ...
        if first_token == "NEWS" or (len(tokens) >= 2 and tokens[1].upper() == "NEWS"):
            tk_news = (
                tokens[1]
                if first_token == "NEWS" and len(tokens) >= 2
                else (tokens[0] if len(tokens) >= 2 and tokens[1].upper() == "NEWS" else top_sym)
            )
        ...
        if (first_token == "PROP" and any(t in ("REBAL", "TRADE", "ORDERS") for t in tokens)) or first_token in (
            "AUTOREBAL",
            "PROPOSAL",
        ):
        ...
        if first_token in ("GLOBAL", "CROSSBORDER", "RESIDENCY", "NEORES") or (
            first_token == "GLOBAL" and "TAX" in tokens
        ):
    ...
    def _cmd_quote(self, ticker: str, ctx: Dict[str, Any]) -> TerminalCommandResult:
        ...
        mkt_cap_str = (
            f"${mkt_cap / 1e12:.2f}T"
            if mkt_cap >= 1e12
            else (f"${mkt_cap / 1e9:.2f}B" if mkt_cap >= 1e9 else f"${mkt_cap / 1e6:.2f}M")
            if mkt_cap > 0
            else "N/A"
        )
        ...
        if not active_pos.empty and "ticker" in active_pos.columns:
            ...
            if not m.empty:
                q_col = (
                    "qty_net"
                    if "qty_net" in m.columns
                    else ("quantity" if "quantity" in m.columns else ("shares" if "shares" in m.columns else None))
                )
                w_col = (
                    "avg_cost"
                    if "avg_cost" in m.columns
                    else ("wacp" if "wacp" in m.columns else ("buy_price" if "buy_price" in m.columns else None))
                )
                v_col = (
                    "current_value"
                    if "current_value" in m.columns
    ...
    def _cmd_watchlist(self, ctx: Dict[str, Any]) -> TerminalCommandResult:
        ...
        lines.extend(["---", "TIP: Digita 'WL ADD <TICKER>' o 'WL DEL <TICKER>' per modifica"])
    ...
    def _cmd_portfolio_live_prices(
        self, df_pos: pd.DataFrame, results: Dict[str, Any], ctx: Optional[Dict[str, Any]] = None
    ) -> TerminalCommandResult:
        ctx = ctx or {}
        port_name = ctx.get("portfolio_name", "Master Wealth")
        active_pos = get_active_positions(df_pos)
        ...
        for _, row in active_pos.iterrows():
            ...
            live_p_eur, spot_orig_str, curr_sym = convert_to_eur(
                live_p_orig, declared_curr, sym, quotes_map, provided_fx
            )
            ...
            lines.append(
                f"{sym:<8} {spot_orig_str:<13} €{live_p_eur:<10.2f} {chg_sign}{chg_1d:<5.1f}%{chg_arrow}  €{wacp_eur:<10.2f} {pnl_str:<26}"
            )
    ...
    def _cmd_close_position(self, tokens: List[str], ctx: Dict[str, Any]) -> TerminalCommandResult:
        ...
        if active_pos.empty or "ticker" not in active_pos.columns:
            return TerminalCommandResult(
                command=f"CLOSE {sym}",
                status="ERROR",
                output_text="Nessun portafoglio attivo per cui chiudere posizioni.",
            )
        ...
        if match.empty:
            return TerminalCommandResult(
                command=f"CLOSE {sym}",
                status="ERROR",
                output_text=f"Ticker '{sym}' non presente tra le posizioni attive del portafoglio.",
            )

        qty_col = (
            "qty_net"
            if "qty_net" in match.columns
            else ("shares" if "shares" in match.columns else ("quantity" if "quantity" in match.columns else None))
        )
        ...
        if q_held <= 1e-6:
            return TerminalCommandResult(
                command=f"CLOSE {sym}",
                status="INFO",
                output_text=f"Nessuna quota attiva (qty={q_held}) per il titolo '{sym}'.",
            )
    ...
    def _cmd_rebalance_summary(
        self, results: Dict[str, Any], df_pos: pd.DataFrame, ctx: Dict[str, Any]
    ) -> TerminalCommandResult:
        port_name = ctx.get("portfolio_name", "Master Wealth")
        active_pos = get_active_positions(df_pos)
        if active_pos.empty or "ticker" not in active_pos.columns:
            return TerminalCommandResult(
                command="REBALANCE",
                status="INFO",
                output_text="Nessun portafoglio attivo collegato per il ribilanciamento.",
            )
        ...
        for _, row in active_pos.iterrows():
            ...
            lines.append(
                f"{sym:<8} € {mv:>12,.2f}    {curr_w * 100:>6.1f}%    {target_w_equal * 100:>6.1f}%     {drift_p:>+6.1f}%    {act:<18}"
            )

        lines.append("---")
        lines.append(
            f"Target Benchmark : 1/N Equal-Risk Baseline ({n_assets} Assets) | Total Capital: € {tot_mkt:,.2f}"
        )
        out_msg = _render_terminal_box(lines, width=90)
        return TerminalCommandResult(command="REBALANCE", status="SUCCESS", output_text=out_msg)

    def _cmd_dividend_summary(
        self, results: Dict[str, Any], df_pos: pd.DataFrame, ctx: Dict[str, Any]
    ) -> TerminalCommandResult:
        port_name = ctx.get("portfolio_name", "Master Wealth")
        active_pos = get_active_positions(df_pos)
        if active_pos.empty or "ticker" not in active_pos.columns:
            return TerminalCommandResult(
                command="DIVIDENDS", status="INFO", output_text="Nessun portafoglio attivo per proiezioni dividendi."
            )
        ...
        tot_val = (
            float(active_pos[val_col].sum()) if val_col else float(results.get("portfolio_value", 100000.0) or 100000.0)
        )
    ...
    def _cmd_trade_order(self, tokens: List[str], raw_cmd: str, ctx: Dict[str, Any]) -> TerminalCommandResult:
        ...
        try:
            ...
            return TerminalCommandResult(
                command=raw_cmd, status="ERROR", output_text=f"Quantità non valida: '{tokens[1]}'"
            )
...
"""
        return TerminalCommandResult(
            command=raw_cmd, status="SUCCESS", output_text=out_msg.strip(), structured_data={"order": new_order}
        )
...
"""
    ...
    def _cmd_cancel_order(self, tokens: List[str], raw_cmd: str) -> TerminalCommandResult:
        if len(tokens) < 2:
            return TerminalCommandResult(
                command=raw_cmd, status="ERROR", output_text="Specifica l'Order ID. Esempio: 'CANCEL ORD-8801'"
            )
        ...
        return TerminalCommandResult(
            command=raw_cmd, status="ERROR", output_text=f"Nessun ordine trovato con ID '{target_id}'."
        )

    def _cmd_sql(self, sql_query: str, ctx: Dict[str, Any]) -> TerminalCommandResult:
        if not duckdb:
            return TerminalCommandResult(
                command="SQL", status="ERROR", output_text="Modulo DuckDB non installato nell'ambiente Python."
            )
        if not sql_query:
            return TerminalCommandResult(
                command="SQL",
                status="INFO",
                output_text="Specifica una query SQL. Esempio: 'SQL SELECT ticker, market_value, pnl_pct FROM df_positions'",
            )
        ...
        try:
            ...
            return TerminalCommandResult(
                command="SQL", status="SUCCESS", output_text=out_msg, structured_data={"df": res_df}
            )
        except Exception as e:
            return TerminalCommandResult(
                command="SQL", status="ERROR", output_text=f"Errore nell'esecuzione SQL DuckDB: {str(e)}"
            )

    def _cmd_eqs(self, eqs_expr: str, ctx: Dict[str, Any]) -> TerminalCommandResult:
        if not evaluate_custom_screener_query:
            return TerminalCommandResult(
                command="EQS", status="ERROR", output_text="Modulo EQS Screener non disponibile."
            )
        if not eqs_expr:
            return TerminalCommandResult(
                command="EQS",
                status="INFO",
                output_text="Specifica una condizione EQS. Esempio: 'EQS Piotroski >= 7 AND Altman > 2.9'",
            )
        ...
        if not isinstance(active_pos, pd.DataFrame) or active_pos.empty:
            return TerminalCommandResult(
                command="EQS", status="INFO", output_text="Nessun dataset di posizioni attive per la valutazione EQS."
            )
        ...
        try:
            ...
            if not is_valid:
                return TerminalCommandResult(
                    command="EQS", status="ERROR", output_text=f"Errore sintassi EQS: {err_msg}"
                )
            ...
            cols_to_show = [
                c
                for c in ["ticker", "company_name", "market_value", "pnl_pct", "argus_score"]
                if c in matches_df.columns
            ]
            table_str = (
                matches_df[cols_to_show].head(10).to_string(index=False)
                if cols_to_show
                else matches_df.head(10).to_string()
            )
            return TerminalCommandResult(
                command="EQS",
                status="SUCCESS",
            ...
            return TerminalCommandResult(
                command="EQS", status="ERROR", output_text=f"Errore di valutazione espressione EQS: {str(e)}"
            )

    def _cmd_var(
        self, tokens: List[str], results: Dict[str, Any], df_pos: pd.DataFrame, ctx: Optional[Dict[str, Any]] = None
    ) -> TerminalCommandResult:
        ctx = ctx or {}
        port_name = ctx.get("portfolio_name", "Master Wealth")
        active_pos = get_active_positions(df_pos)
        ...
        if conf == "99%":
            var_pct = float(
                results.get("historical_var_99", results.get("var_99_pct", var_pct * 1.41)) or var_pct * 1.41
            )
    ...
    def _cmd_metric(
        self, metric_name: str, results: Dict[str, Any], ctx: Optional[Dict[str, Any]] = None
    ) -> TerminalCommandResult:
        ctx = ctx or {}
        port_name = ctx.get("portfolio_name", "Master Wealth")
        ...
        return TerminalCommandResult(
            command=metric_name, status="SUCCESS", output_text=f"[{port_name}] {metric_name} Portfolio Level: {val_str}"
        )

    def _cmd_correlation(
        self, tokens: List[str], df_ret: pd.DataFrame, df_pos: Optional[pd.DataFrame] = None
    ) -> TerminalCommandResult:
        ...
        if len(tokens) == 2:
            ...
            if not isinstance(df_ret, pd.DataFrame) or df_ret.empty:
                return TerminalCommandResult(
                    command="CORR",
                    status="INFO",
                    output_text="Serie storiche dei rendimenti non caricate nella sessione.",
                )
            ...
            if not col1:
                return TerminalCommandResult(
                    command="CORR",
                    status="ERROR",
                    output_text=f"Ticker '{t1}' non presente nelle serie storiche dei rendimenti.",
                )
            ...
            if not active_pos.empty and "ticker" in active_pos.columns:
                target_syms = [
                    str(t).strip().upper() for t in active_pos["ticker"].unique() if str(t).strip().upper() != t1
                ]
                ...
                sub_cols = [
                    ...
                    if c != col1 and c not in ("Date", "Benchmark", "BENCHMARK", "SPY", "^GSPC")
                ]
            ...
            if not sub_cols:
                return TerminalCommandResult(
                    command="CORR",
                    status="INFO",
                    output_text=f"Nessun altro asset attivo trovato con cui calcolare la correlazione per '{t1}'.",
                )
            ...
            if not corr_items:
                return TerminalCommandResult(
                    command="CORR",
                    status="ERROR",
                    output_text=f"Dati storici sovrapposti insufficienti per correlare '{t1}' con altri asset.",
                )
        ...
        if not isinstance(df_ret, pd.DataFrame) or df_ret.empty:
            return TerminalCommandResult(
                command="CORR", status="INFO", output_text="Serie storiche rendimenti non caricate nella sessione."
            )
        ...
        if len(cols) < 2:
            return TerminalCommandResult(
                command="CORR",
                status="ERROR",
                output_text=f"Uno o entrambi i ticker ({t1}, {t2}) non sono presenti nella matrice dei rendimenti.",
            )
        ...
        if len(common_idx) < 10:
            return TerminalCommandResult(
                command="CORR",
                status="ERROR",
                output_text="Dati storici insufficienti per il calcolo della correlazione.",
            )
...
"""
    ...
    def _cmd_health_score(
        self, results: Dict[str, Any], df_pos: pd.DataFrame, ctx: Optional[Dict[str, Any]] = None
    ) -> TerminalCommandResult:
        ctx = ctx or {}
        port_name = ctx.get("portfolio_name", "Master Wealth")
        active_pos = get_active_positions(df_pos)
        ...
        verdict = (
            "HEALTHY & COMPLIANT [OK]"
            if score >= 75
            else ("MONITOR ATTENTION [WARN]" if score >= 50 else "DISTRESS RISK [ALERT]")
        )
        ...
        tot_val = (
            float(active_pos[val_col].sum())
            if val_col and not active_pos.empty
            else float(results.get("portfolio_value", 0.0) or 0.0)
        )
...
"""
    ...
    def _cmd_stream_summary(self, tokens: List[str]) -> TerminalCommandResult:
        ...
        if not buf:
            return TerminalCommandResult(
                command="STREAM", status="INFO", output_text="Modulo streaming in-memory attivo."
            )
...
"""
    ...
    def _cmd_news(self, ticker: str) -> TerminalCommandResult:
        ...
        lines = [f"FINANCIAL NEWS STREAM & MARKET SENTIMENT: {sym:<44}", "---"]
        ...
        for idx, item in enumerate(raw_news[:4], 1):
            ...
            if any(
                w in lower_title
                for w in ["record", "surge", "gain", "profit", "beat", "rally", "upgrade", "bull", "growth", "high"]
            ):
                sent_badge = "[+] BULLISH "
            elif any(
                w in lower_title
                for w in ["drop", "fall", "slump", "miss", "loss", "plunge", "downgrade", "bear", "lawsuit", "cut"]
            ):
                sent_badge = "[-] BEARISH "
            else:
                sent_badge = "[~] NEUTRAL "
    ...
    def _cmd_shock(self, tokens: List[str], ctx: Dict[str, Any]) -> TerminalCommandResult:
        ...
        if not isinstance(active_pos, pd.DataFrame) or active_pos.empty or "ticker" not in active_pos.columns:
            return TerminalCommandResult(
                command="SHOCK",
                status="ERROR",
                output_text="Portafoglio privo di posizioni attive. Impossibile simulare lo stress test.",
            )
        ...
        for _, row in active_pos.iterrows():
            ...
            lines.append(
                f"{sym:<8} € {val_eur:>11,.2f}     € {shocked_val:>13,.2f}     {d_sgn}€ {delta_eur:>11,.2f}     {pct_val:>+6.1f}%"
            )
    ...
    def _cmd_snap(self, ctx: Dict[str, Any]) -> TerminalCommandResult:
        ...
        if not isinstance(active_pos, pd.DataFrame) or active_pos.empty or "ticker" not in active_pos.columns:
            return TerminalCommandResult(
                command="SNAP", status="INFO", output_text="Nessuna posizione attiva per cui generare lo snapshot."
            )
        ...
        for _, row in active_pos.iterrows():
            ...
            chg_1d = q["change_pct"]
            lines.append(
                f"{sym},{decl_curr},{live_p:.2f},{live_p_eur:.2f},{qty:.2f},{val_eur:.2f},{wacp:.2f},{pnl_eur:.2f},{chg_1d:.2f}"
            )
    ...
    def _cmd_corr_matrix(self, df_ret: pd.DataFrame, df_pos: Optional[pd.DataFrame] = None) -> TerminalCommandResult:
        if not isinstance(df_ret, pd.DataFrame) or df_ret.empty or len(df_ret.columns) < 2:
            return TerminalCommandResult(
                command="CORR MATRIX",
                status="ERROR",
                output_text="Serie storiche insufficienti per costruire la matrice di correlazione.",
            )
        ...
        if not active_pos.empty and "ticker" in active_pos.columns:
            ...
            for c in df_ret.columns:
                ...
                if c_up in p_ticks or any(
                    p == c_up or c_up.startswith(p + ".") or p.startswith(c_up + ".") for p in p_ticks
                ):
        ...
        if len(sub_cols) < 2:
            sub_cols = [
                c for c in df_ret.columns if c not in ["Date", "DATE", "date", "Benchmark", "BENCHMARK", "SPY", "^GSPC"]
            ]
        if len(sub_cols) < 2:
        ...
        if len(sub_cols) < 2:
            return TerminalCommandResult(
                command="CORR MATRIX",
                status="ERROR",
                output_text="Almeno 2 asset necessari per costruire la matrice di correlazione.",
            )
        ...
        for row_c in sub_cols:
            vals = [
                f"{corr.loc[row_c, col_c]:>+6.2f}" if not np.isnan(corr.loc[row_c, col_c]) else "   N/A"
                for col_c in sub_cols
            ]
    ...
    def _cmd_wealth_goals(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            for _, g in goals_df.iterrows():
                ...
                lines.append(
                    f"{str(g.get('name'))[:24]:<25} | € {cur:>9,.0f} | € {tgt:>9,.0f} | {pct:>6.1f}% | € {pac:>7,.0f}"
                )
...
"""
    ...
    def _cmd_wealth_drift(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            cash_alert_str = (
                f"ATTIVO 🔴 (Eccesso € {w.get('excess_cash_eur', 0.0):,.2f})"
                if w.get("cash_drag_alert")
                else "REGOLARE 🟢"
            )
            lines = [
                "[TAX-SMART REBALANCING WATCHDOG & DRIFT MONITOR]",
            ...
            for d in w.get("drift_table", []):
                lines.append(
                    f"{d['asset_name'][:19]:<20} | {d['current_weight_pct']:>7.1f}% | {d['target_weight_pct']:>7.1f}% | {d['drift_pct']:>+7.1f}% | {d['action_type']} (€ {abs(d['target_delta_eur']):,.0f})"
                )
...
"""
    ...
    def _cmd_wealth_holding(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[FAMILY OFFICE MULTI-ENTITY CONSOLIDATOR & PEX TAX DESK]",
            ...
            for e in fo.get("entities_detail", []):
                lines.append(
                    f"{e['name'][:29]:<30} | € {e['consolidated_net_equity_eur']:>17,.2f} | {e.get('weight_on_consolidated_pct', 0.0):>6.1f}% | {e['effective_tax_rate_est']:.1f}%"
                )
    ...
    def _cmd_wealth_pe(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[PRIVATE EQUITY, VENTURE CAPITAL & REAL ASSETS DESK]",
            ...
            for d in pe.get("deals_list", []):
                lines.append(
                    f"{d['name'][:27]:<28} | {d['asset_class'][:15]:<16} | € {d['called_capital_eur']:>8,.0f} | € {d['current_nav_estimated_eur']:>8,.0f} | {d['moic_multiple']:>5.2f}x | {d['irr_net_pct']:>5.1f}%"
                )
    ...
    def _cmd_wealth_fx_hedge(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[MULTI-CURRENCY FX EXPOSURE & FORWARD HEDGING OVERLAY]",
            ...
            for it in fx.get("exposures_list", []):
                lines.append(
                    f"{it['currency']:<8} | € {it['nominal_amount_eur']:>11,.0f} | {it['weight_pct']:>6.1f}% | {it['local_interest_rate_pct']:>10.2f}% | {it['annual_forward_points_cost_pct']:>+10.2f}% | {it['hedged_ratio_pct']:>6.1f}%"
                )
    ...
    def _cmd_wealth_govern(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[FAMILY GOVERNANCE & PATTO DI FAMIGLIA (ART. 768-BIS C.C.)]",
            ...
            for h in gov.get("non_assigned_heirs", []):
                lines.append(
                    f"{h['heir_name']:<30} | {h['statutory_legitimate_share_pct']:>12.1f}% | € {h['compensation_due_eur']:>19,.2f}"
                )
    ...
    def _cmd_wealth_brinson(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[TOTAL WEALTH BRINSON-FACHLER MULTI-ASSET ATTRIBUTION]",
            ...
            for b in br.get("breakdown_list", []):
                lines.append(
                    f"{b['asset_class'][:31]:<32} | {b['portfolio_weight_pct']:>6.1f}% | {b['benchmark_weight_pct']:>6.1f}% | {b['portfolio_return_pct']:>+6.1f}% | {b['benchmark_return_pct']:>+6.1f}% | {b['total_contribution_pct']:>+9.2f}%"
                )
            return TerminalCommandResult(command="ATTR WEALTH", status="SUCCESS", output_text="\n".join(lines))
        except Exception as e:
            return TerminalCommandResult(
                command="ATTR WEALTH", status="ERROR", output_text=f"Errore Brinson Wealth: {e}"
            )
    ...
    def _cmd_wealth_recon(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[SMART CASHFLOW RECONCILIATION & AUTO-MATCHING ENGINE]",
            ...
            for m in rc.get("matches_list", [])[:10]:
                ...
                lines.append(
                    f"{m['tx_date']:<11} | € {m['amount_eur']:>9,.2f} | {m['matched_category'][:24] + dupe_flag:<25} | {m['match_confidence_pct']:>5.0f}% | {m['match_source']}"
                )
    ...
    def _cmd_macro_stress(
        self, results: Dict[str, Any], df_pos: Optional[pd.DataFrame] = None, ctx: Optional[Dict[str, Any]] = None
    ) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[INSTITUTIONAL MACRO FACTOR STRESS TESTING (EBA & FED CCAR)]",
            ...
            for sc in st_res.get("scenario_results", []):
                lines.append(
                    f"{sc['scenario_name'][:31]:<32} | {sc['equity_shock_pct']:>+7.1f}% | {sc['rate_shock_bps']:>+9.0f} | {sc['portfolio_return_pct']:>+12.2f}% | € {sc['pnl_impact_eur']:>10,.2f}"
                )
            return TerminalCommandResult(command="STRESS MACRO", status="SUCCESS", output_text="\n".join(lines))
        except Exception as e:
            return TerminalCommandResult(
                command="STRESS MACRO", status="ERROR", output_text=f"Errore Macro Stress: {e}"
            )

    def _cmd_reverse_stress(
        self,
        results: Dict[str, Any],
        df_pos: Optional[pd.DataFrame] = None,
        ...
        try:
            ...
            lines = [
                "[REVERSE STRESS TESTING & INSOLVENCY SOLVER]",
                ...
                "",
                "Soglie Minime per Innesco Drawdown Target:",
    ...
    def _cmd_rebal_proposal(
        ...
        try:
            ...
            lines = [
                "[AUTONOMOUS REBALANCING & TRADE PROPOSAL GENERATOR]",
            ...
            for tr in reb.get("trades_list", []):
                lines.append(
                    f"{tr['ticker']:<10} | {tr['action']:<6} | {tr['current_weight_pct']:>7.1f}% | {tr['target_weight_pct']:>7.1f}% | {tr['suggested_shares']:>8} | € {tr['trade_notional_eur']:>14,.2f}"
                )
            return TerminalCommandResult(command="PROP REBAL", status="SUCCESS", output_text="\n".join(lines))
        except Exception as e:
            return TerminalCommandResult(
                command="PROP REBAL", status="ERROR", output_text=f"Errore Rebal Proposal: {e}"
            )

    def _cmd_mifid_check(
        self, results: Dict[str, Any], df_pos: Optional[pd.DataFrame] = None, ctx: Optional[Dict[str, Any]] = None
    ) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[MIFID II SUITABILITY & CONCENTRATION GATE]",
            ...
            if mif["violations"]:
                ...
                for v in mif["violations"]:
                    lines.append(
                        f"  ❌ [{v['type']}] Ticker: {v.get('ticker', 'N/D')} -> Peso {v.get('weight_pct', 0)}% vs Limite {v.get('limit_pct', 0)}%"
                    )
            if mif["warnings"]:
    ...
    def _cmd_esg_summary(
        ...
        try:
            ...
            for h in esg.get("holdings_esg_list", [])[:8]:
                lines.append(
                    f"{h['ticker']:<10} | {h['name'][:23]:<24} | {h['weight_pct']:>5.1f}% | {h['esg_score']:>5.1f} | {h['sfdr_classification']:<8} | {h['carbon_intensity_tco2e']:>11.1f}"
                )
    ...
    def _cmd_options_payoff(
        self, tokens: Optional[List[str]] = None, ctx: Optional[Dict[str, Any]] = None
    ) -> TerminalCommandResult:
        try:
            ...
            if tokens and len(tokens) > 1:
                ...
                elif "SPREAD" in t_sub:
                    strat = "Bull Call Spread"
                elif "STRADDLE" in t_sub:
                    strat = "Long Straddle"
                elif "COVER" in t_sub:
                    strat = "Covered Call"
            ...
            gr = opt["greeks"]
            lines = [
                "[OPTIONS STRATEGY WORKBENCH & PAYOFF DESK]",
            ...
            for leg in opt.get("legs", []):
                lines.append(
                    f"{leg['leg_type']:<6} | {leg['action']:<6} | € {leg['strike_eur']:>9.2f} | € {leg['premium_unit_eur']:>13.2f} | {leg['delta']:>+6.3f}"
                )
            return TerminalCommandResult(command="OPTS BUILD", status="SUCCESS", output_text="\n".join(lines))
        except Exception as e:
            return TerminalCommandResult(
                command="OPTS BUILD", status="ERROR", output_text=f"Errore Options Workbench: {e}"
            )
    ...
    def _cmd_report_qtr(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            pdf_bytes = generate_white_label_quarterly_pdf_report(
                engine, portfolio_id=1, client_name="Family Office Master", quarter="Q1 2026"
            )
            lines = [
                "[WHITE-LABEL CLIENT QUARTERLY PDF REPORT GENERATOR]",
                "Status Generazione            : COMPLETATA CON SUCCESSO 🟢",
                f"Dimensione File PDF Generato  : {len(pdf_bytes):,} bytes ({len(pdf_bytes) / 1024:.1f} KB)",
                "Moduli Inclusi nel Dossier    : Bilancio Net Worth, Brinson Multi-Asset, EBA Stress Test, SFDR ESG Scorecard",
                "Pronto per il Download        : Disponibile da interfaccia Streamlit (Modulo 21 AI Copilot / Modulo 13 Net Worth).",
            ]
    ...
    def _cmd_private_debt(
        ...
        try:
            ...
            lines = [
                "[PRIVATE DEBT, DIRECT LENDING & COVENANTS DESK]",
            ...
            for tr in pd_res.get("tranches_list", []):
                lines.append(
                    f"{tr['tranche_name'][:27]:<28} | {tr['seniority']:>3} | € {tr['notional_eur']:>10,.0f} | {tr['cash_coupon_pct']:>5.2f}% | {tr['pik_coupon_pct']:>4.2f}% | {tr['attachment_leverage']:>5.2f}x | {tr['detachment_leverage']:>5.2f}x"
                )
    ...
    def _cmd_execution_algo(
        ...
        try:
            ...
            if tokens and len(tokens) > 1 and tokens[1] not in ("EXEC", "IMPACT"):
                tk = tokens[1].upper()
            ex = compute_implementation_shortfall_and_execution_benchmarks(
                ticker=tk, total_shares=5000, decision_price=100.0
            )
            pb = ex["perold_breakdown"]
            lines = [
                "[ALGORITHMIC TRADE EXECUTION & IMPLEMENTATION SHORTFALL (PEROLD 1988)]",
            ...
            for st in ex.get("strategies_comparison", []):
                lines.append(
                    f"{st['strategy'][:35]:<36} | € {st['avg_exec_price_eur']:>13.3f} | {st['slippage_bps']:>6.1f} bps | € {st['total_execution_cost_eur']:>10,.2f}"
                )
    ...
    def _cmd_cross_border_tax(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            cb = compute_cross_border_wealth_tax_comparison(
                total_wealth_eur=10000000.0, annual_capital_gain_eur=400000.0, annual_foreign_income_eur=250000.0
            )
            lines = [
                "[CROSS-BORDER TAX & GLOBAL WEALTH STRUCTURING]",
            ...
            for r in cb.get("comparison_list", []):
                lines.append(
                    f"{r['name'][:34]:<35} | € {r['annual_cgt_eur']:>7,.0f} | € {r['annual_income_tax_eur']:>7,.0f} | € {r['annual_wealth_tax_eur']:>7,.0f} | € {r['total_annual_tax_eur']:>13,.2f} | {r['effective_annual_tax_rate_pct']:>6.1f}%"
                )
            return TerminalCommandResult(command="GLOBAL TAX", status="SUCCESS", output_text="\n".join(lines))
        except Exception as e:
            return TerminalCommandResult(
                command="GLOBAL TAX", status="ERROR", output_text=f"Errore Cross-Border Tax: {e}"
            )
    ...
    def _cmd_hmm_regime(
        ...
        try:
            ...
            lines = [
                "[MACHINE LEARNING HIDDEN MARKOV MODELS (HMM) REGIME DETECTOR]",
            ...
            for st in hmm.get("state_profiles", []):
                lines.append(
                    f"{st['state_name'][:25]:<26} | {st['frequency_pct']:>5.1f}% | {st['annualized_return_pct']:>+11.2f}% | {st['annualized_volatility_pct']:>11.2f}% | {st['sharpe_ratio']:>6.2f} | {st['persistence_prob_pct']:>7.1f}%"
                )
    ...
    def _cmd_voice_brief(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            lines = [
                "[AI VOICE EXECUTIVE BRIEFING & AUDIO PODCAST]",
                ...
                "Format Trasmissione            : Copione a 2 Voci (CIO & Chief Risk Officer)",
                "",
                "Estratto Script Vocale:",
            ]
    ...
    def _cmd_wealth_temporal(self, ctx: Optional[Dict[str, Any]] = None) -> TerminalCommandResult:
        try:
            ...
            lines = [
                ...
                "",
                "Dettaglio Traiettoria Recente:",
            ]
            for idx, r in prog["history_df"].tail(4).iterrows():
                dt_str = str(idx.date() if hasattr(idx, "date") else idx)
                lines.append(
                    f"  📅 {dt_str}: Net Worth € {r['total_net_worth']:>11,.0f} | Liq € {r['liquid_cash']:>9,.0f} | Invest € {r['financial_investments']:>9,.0f} | Immobili € {r['real_estate']:>9,.0f}"
                )
            return TerminalCommandResult(command="WEALTH TIME", status="SUCCESS", output_text="\n".join(lines))
        except Exception as e:
            return TerminalCommandResult(
                command="WEALTH TIME", status="ERROR", output_text=f"Errore Wealth Temporal: {e}"
            )
