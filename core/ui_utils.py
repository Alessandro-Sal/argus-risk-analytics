...
def render_command_bar():
    """Renderizza la barra di stato e comando ARGUS v5.4.0 in cima alla pagina con telemetria, spotlight e popout 2° monitor."""
    try:
...
def render_splash_screen(force_show: bool = False) -> bool:
    ...
    html_content = (
        ...
        f'<div style="color:{accent};font-weight:bold;">[●] ARGUS Terminal v5.4.0 Ready for Operations</div>'
        f'</div>'
        f'</div>'
    )
...
def render_control_room_hero():
    ...
    hero_html = (
        ...
        f'<span style="font-size:10px;font-weight:700;color:{accent};background:rgba(255,153,0,0.1);padding:2px 8px;border-radius:12px;">v5.4.0</span>'
        f'</div>'
