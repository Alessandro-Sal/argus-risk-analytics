...
class GenerationalTransferOptimizer:
    ...
    @staticmethod
    def compute_relictum_donatum_balance(relictum_gross: float, liabilities: float, donatum: float) -> Dict[str, float]:
        """
    ...
    @classmethod
    def compute_statutory_shares(cls, estate_total: float, family_profile: FamilyProfile) -> SuccessionSharesResult:
        """
    ...
    def check_reduction_risk(
        ...
        return {
            ...
            if injury_detected
            else ["Nessuna lesione di legittima rilevata. Asse ereditario conforme."],
        }
    ...
    def simulate_generational_plan(
        ...
        if n_recipients > 0:
            for item in shares_ante.legitimate_shares:
                ...
                bare_share_per_heir = (
                    bare_ownership_val / max(1, family_profile.children_count) if item.relationship == "figlio" else 0.0
                )
        ...
        if not recommendations:
            recommendations.append(
                "Attivare le leve di pianificazione (Polizze, Nuda Proprietà o Patto di Famiglia) per simulare l'abbattimento fiscale."
            )
    ...
    def generate_executive_succession_memo(
        cls, comparison: OptimizationComparisonResult, family_profile: FamilyProfile
    ) -> str:
        """
