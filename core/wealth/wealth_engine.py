...
def compute_consolidated_net_worth(
    engine: Engine, portfolio_id: Optional[int] = None, risk_portfolio_ids: Optional[List[int]] = None
) -> NetWorthSummary:
    """
    ...
    if not df_acc.empty:
        brokerage_types = [
            AccountType.BROKERAGE_CASH.value,
            "brokerage",
            "crypto_exchange",
            "trading",
            "investment",
    ...
    total_assets = summary.liquid_cash + summary.financial_investments + summary.physical_assets + summary.pension_total
...
def compute_cashflow_analytics(df_cf: pd.DataFrame) -> Dict[str, Any]:
    ...
    savings_amount = float(
        df[(df["nature"] == CategoryNature.SAVING_INVESTMENT.value) & (~is_transfer)]["amount"].sum()
    ) + max(0.0, net_savings)
    ...
    monthly_df = (
        df_real.groupby(["year_month", "direction"])["amount"].sum().unstack(fill_value=0.0).reset_index()
        if not df_real.empty
        else pd.DataFrame()
    )
    if not monthly_df.empty:
        if "inflow" not in monthly_df.columns:
            monthly_df["inflow"] = 0.0
        if "outflow" not in monthly_df.columns:
            monthly_df["outflow"] = 0.0
        monthly_df["net"] = monthly_df["inflow"] - monthly_df["outflow"]
        monthly_df["savings_rate_pct"] = np.where(
            monthly_df["inflow"] > 0, (monthly_df["net"] / monthly_df["inflow"]) * 100.0, 0.0
        )
...
def _build_sankey_data(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    inflows = (
        clean_df[clean_df["direction"] == "inflow"]
    ...
    if tot_inflow > 0:
        ...
        for cname, amt in inflows.items():
            if amt >= tot_inflow * 0.04:  # Soglia minima 4%
                ...
                links.append(
                    {"source": idx, "target": hub_idx, "value": float(amt), "color": "rgba(16, 185, 129, 0.35)"}
                )
            else:
        ...
        if other_inflow_sum > 0:
            ...
            links.append(
                {"source": idx, "target": hub_idx, "value": other_inflow_sum, "color": "rgba(20, 184, 166, 0.3)"}
            )
    ...
    for p_name, p_sum, p_col, p_link_col, p_df in pillars:
        if p_sum > 0:
            ...
            links.append({"source": hub_idx, "target": p_idx, "value": p_sum, "color": p_link_col})
            ...
            if other_sub_sum > 0:
                ...
                links.append(
                    {"source": p_idx, "target": c_idx, "value": other_sub_sum, "color": "rgba(255, 255, 255, 0.1)"}
                )
...
def compute_wealth_health_score(summary: NetWorthSummary, cf_analytics: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"score": final_score, "rating": rating, "breakdown": breakdown}
...
def compute_mortgage_amortization(
    ...
    if monthly_rate > 0:
        base_monthly = (
            principal * (monthly_rate * (1 + monthly_rate) ** total_months) / ((1 + monthly_rate) ** total_months - 1)
        )
    else:
    ...
    rate_shocks = {}
    for shock_bps, shock_label in [
        (100, "+1.00% (Euribor +100bps)"),
        (200, "+2.00% (Shock Restrittivo)"),
        (-100, "-1.00% (Taglio Tassi)"),
    ]:
...
def compute_estate_planning_analytics(
    ...
    if has_spouse and children_count == 0 and not has_ascendants:
        ...
        quota_desc = (
            "Coniuge senza figli (art. 540 c.c.): 50% Legittima Coniuge, 50% Disponibile (+ Diritto Abitazione)."
        )
...
def compute_ai_wealth_diagnostics(
    engine, portfolio_id: int = 1, target_model_name: str = "🏦 Bilanciato Istituzionale (60/40 Equity/Bond)"
) -> Dict[str, Any]:
    """
...
def generate_executive_tear_sheet_html(engine, portfolio_id: int = 1) -> str:
    ...
    <body>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr style="background: #f1f5f9; font-weight: bold;">
                    ...
                    <td>&euro; {
        (nw.liquid_cash + nw.financial_investments + nw.physical_assets + nw.pension_total):,.2f}</td>
                    <td>100.0%</td>
                    <td>Consolidato</td>
                </tr>
            </tbody>
        ...
        if not df_risk_linked.empty
        else "<tr><td colspan='3' style='text-align:center; color:#94a3b8;'>Nessun portafoglio collegato o sincronizzazione attiva</td></tr>"
    }
            </tbody>
        </table>
...
def generate_executive_tear_sheet_pdf(engine, portfolio_id: int = 1) -> bytes:
    ...
    story = [
        ...
        Paragraph(
            f"<b>Liquidit&agrave;:</b> &euro; {nw.liquid_cash:,.2f} &bull; <b>Investimenti:</b> &euro; {nw.financial_investments:,.2f}",
            styles["Normal"],
        ),
        Paragraph(
            f"<b>Caveau &amp; Fisico:</b> &euro; {nw.physical_assets:,.2f} &bull; <b>Previdenza:</b> &euro; {nw.pension_total:,.2f}",
            styles["Normal"],
        ),
        Paragraph(
            f"<b>Wealth Health Score:</b> {nw.wealth_health_score:.0f}/100 &bull; <b>Runway:</b> {nw.runway_months:.1f} Mesi",
            styles["Normal"],
        ),
        Spacer(1, 20),
...
def compute_recurring_subscriptions_analytics(
    df_tx: Optional[pd.DataFrame] = None, engine: Optional[Engine] = None, portfolio_id: int = 1
) -> Dict[str, Any]:
    """
    ...
    if not detected_subs and df_tx is not None and not df_tx.empty:
        ...
        if not df_exp.empty:
            ...
            for merch, group in df_exp.groupby(merchant_col):
                ...
                if any(ex in cat_lower for ex in excluded_category_keywords) or any(
                    ex in merch_lower for ex in excluded_merchants
                ):
                    continue
    ...
    if not detected_subs:
        sample_subs = [
            {
                "merchant": "SaaS & Strumenti Cloud",
                "category": "Software & Cloud",
                "cadence": "Mensile",
                "amount": 35.00,
                "monthly_amount": 35.00,
                "annual_amount": 420.00,
                "payment_day": 1,
                "start_date": None,
                "end_date": None,
                "last_date": "Live",
                ...
                "is_split": False,
                "split_details": None,
                "opportunity_cost_5y": 2505.0,
                "opportunity_cost_10y": 6058.0,
                "opportunity_cost_20y": 18233.0,
            },
            {
                "merchant": "Streaming & Musica",
                "category": "Streaming & Intrattenimento",
                "cadence": "Mensile",
                "amount": 28.98,
                "monthly_amount": 28.98,
                "annual_amount": 347.76,
                "payment_day": 5,
                "start_date": None,
                "end_date": None,
                "last_date": "Live",
                ...
                "is_split": False,
                "split_details": None,
                "opportunity_cost_5y": 2074.0,
                "opportunity_cost_10y": 5016.0,
                "opportunity_cost_20y": 15096.0,
            },
            {
                "merchant": "Fibra Ottica & Mobile",
                "category": "Telefonia & Internet",
                "cadence": "Mensile",
                "amount": 39.90,
                "monthly_amount": 39.90,
                "annual_amount": 478.80,
                "payment_day": 15,
                "start_date": None,
                "end_date": None,
                "last_date": "Live",
                ...
                "is_split": False,
                "split_details": None,
                "opportunity_cost_5y": 2856.0,
                "opportunity_cost_10y": 6906.0,
                "opportunity_cost_20y": 20786.0,
            },
        ]
...
def compute_cashflow_forecast_and_anomalies(
    df_tx: pd.DataFrame, current_liquid_cash: float = 15000.0
) -> Dict[str, Any]:
    """
                                ...
                                anomalies.append(
                                    {
                                        ...
                                        "categoria": str(cat),
                                        "descrizione": str(
                                            row.get("merchant", row.get("notes", "Spesa Straordinaria"))
                                        ),
                                        "importo": round(val, 2),
        ...
        nwar_results.append(
            {
                ...
                "runway_post_shock": round(
                    shk_cash / max(1.0, nw.monthly_burn_rate * (1.0 + sc_params["burn_rate_shock"])), 1
                ),
            }
        )
...
def compute_merchant_pareto_analytics(df_cf: pd.DataFrame) -> Dict[str, Any]:
    ...
    cat_ser = (
        df_cf["category_name"].astype(str) if "category_name" in df_cf.columns else pd.Series("", index=df_cf.index)
    )
...
def compute_seasonality_matrix(df_cf: pd.DataFrame) -> pd.DataFrame:
    ...
    is_inv = (cat_ser.str.contains("investiment|titoli|azioni|criptovalut|crypto", case=False, na=False)) | (
        notes_ser.str.contains(r"\[investment\]|pac |degiro|binance", case=False, na=False)
    )
    df = df_cf[(dir_ser == "outflow") & (~is_tr) & (~is_inv)].copy()
    if df.empty:
    ...
    pivot = df.pivot_table(index="category_name", columns="month", values="amount", aggfunc="sum", fill_value=0.0)
...
def compute_envelope_budget_analytics(
    df_cf_period: pd.DataFrame, custom_budgets: Dict[str, float] = None, df_cf_historical: pd.DataFrame = None
) -> pd.DataFrame:
    """
    ...
    notes_ser = (
        df_cf_period["notes"].astype(str)
        if "notes" in df_cf_period.columns
        else pd.Series("", index=df_cf_period.index)
    )
    cat_ser = (
        df_cf_period["category_name"].astype(str)
        if "category_name" in df_cf_period.columns
        else pd.Series("", index=df_cf_period.index)
    )
    nat_ser = (
        df_cf_period["nature"].astype(str)
        if "nature" in df_cf_period.columns
        else pd.Series("", index=df_cf_period.index)
    )
    dir_ser = (
        df_cf_period["direction"].astype(str)
        if "direction" in df_cf_period.columns
        else pd.Series("", index=df_cf_period.index)
    )
    ...
    is_inflow_cat = (
        cat_ser.str.contains(
            "entrata|stipendio|bonus|introiti|borse di studio|supporto famiglia|compensi|dividendi",
            case=False,
            na=False,
        )
...
def compute_cashflow_whatif_reinvestment(
    monthly_savings_boost: float, annual_return_rate: float = 0.07, max_years: int = 30
) -> Dict[str, Any]:
    """
...
def compute_dynamic_glide_path(
    years_to_target: float, total_horizon_years: float = 20.0, risk_profile: str = "moderate"
) -> Dict[str, Any]:
    """
...
def compute_portfolio_tco_and_fee_drag(
    ...
    if df_positions is not None and not df_positions.empty:
        ...
        val_col = (
            "market_value"
            if "market_value" in df_positions.columns
            else (
                "current_value"
                if "current_value" in df_positions.columns
                else ("total_value" if "total_value" in df_positions.columns else None)
            )
        )
        if val_col:
    ...
    for yr in holding_years:
        ...
        # FV Lordo (Zero Costi)
        fv_gross = w0 * ((1.0 + r_gross_m) ** n_m) + c0 * (
            ((1.0 + r_gross_m) ** n_m - 1.0) / r_gross_m if r_gross_m > 0 else n_m
        )
        # FV Portafoglio Reale
        fv_port = w0 * ((1.0 + r_port_m) ** n_m) + c0 * (
            ((1.0 + r_port_m) ** n_m - 1.0) / r_port_m if r_port_m > 0 else n_m
        )
        # FV Benchmark Low-Cost ETF (0.15%)
        fv_bench = w0 * ((1.0 + r_bench_m) ** n_m) + c0 * (
            ((1.0 + r_bench_m) ** n_m - 1.0) / r_bench_m if r_bench_m > 0 else n_m
        )
    ...
    return {
        ...
        "drag_10y_eur": float(df_comp.loc[df_comp["years"] == 10, "fee_drag_total_eur"].values[0])
        if 10 in df_comp["years"].values
        else 0.0,
        "drag_20y_eur": float(df_comp.loc[df_comp["years"] == 20, "fee_drag_total_eur"].values[0])
        if 20 in df_comp["years"].values
        else 0.0,
        "drag_30y_eur": float(df_comp.loc[df_comp["years"] == 30, "fee_drag_total_eur"].values[0])
        if 30 in df_comp["years"].values
        else 0.0,
    }
...
def compute_advanced_estate_planning(
    ...
    for h in heirs:
        ...
        if is_dis:
            franchise = 1500000.0
            tax_rate = (
                0.04 if rel in ["spouse", "child", "parent"] else (0.06 if rel in ["sibling", "relative_4th"] else 0.08)
            )
...
def compute_tax_smart_rebalancing_watchdog(
    ...
    current_buckets = {
        ...
        "Metalli Preziosi & Orologi": nw.luxury_watches_total
        + nw.precious_metals_total
        + max(0.0, nw.physical_assets - nw.real_estate_total - nw.luxury_watches_total - nw.precious_metals_total),
...
def compute_real_estate_net_equity_and_ltv(engine: Engine, portfolio_id: int = 1) -> Dict[str, Any]:
    """
    ...
    if not df_phys.empty:
        cat_col = (
            "asset_category"
            if "asset_category" in df_phys.columns
            else ("category" if "category" in df_phys.columns else None)
        )
        if cat_col:
    ...
    val_col = (
        ...
        if "current_market_value" in df_re.columns
        else ("current_value" if "current_value" in df_re.columns else None)
    )
...
def generate_advisory_pitchbook_html(engine: Engine, portfolio_id: int = 1) -> str:
    ...
    for d in rebalance.get("drift_table", []):
        color = (
            "#ef4444"
            if d["drift_status"] == "CRITICAL"
            else ("#f59e0b" if d["drift_status"] == "MODERATE" else "#10b981")
        )
    ...
    health_grade = (
        "Eccellente (A+)"
        if nw.wealth_health_score >= 85
        else (
            "Solido (A)"
            if nw.wealth_health_score >= 70
            else ("Migliorabile (B)" if nw.wealth_health_score >= 50 else "Critico (C)")
        )
    )
    fire_target = (
        (nw.monthly_burn_rate * 12.0 * 25.0)
        if nw.monthly_burn_rate > 0
        else (nw.liquid_cash + nw.financial_investments) * 1.5
    )
    if fire_target <= 0:
...
def generate_advisory_pitchbook_pdf(engine: Engine, portfolio_id: int = 1) -> bytes:
    ...
    story = [
        ...
        Paragraph(
            f"<b>Immobili:</b> &euro; {nw.real_estate_total:,.2f} &bull; <b>Passivit&agrave;:</b> &euro; {nw.total_liabilities:,.2f}",
            styles["Normal"],
        ),
        Spacer(1, 15),
...
def compute_ai_quarterly_wealth_review(
    ...
    runway_status = (
        "Ampia (> 24 mesi)"
        if nw.runway_months >= 24
        else ("Adeguata (12-24 mesi)" if nw.runway_months >= 12 else "In Tensione (< 12 mesi)")
    )
...
Le proiezioni stocastiche confermano un'elevata probabilità di successo per i pilastri di medio termine, supportati da apporti periodici programmati di **€ {float(goals_df["monthly_contribution"].sum()):,.2f}/mese**."""
    ...
    macro_text = """Nel contesto macroeconomico attuale caratterizzato dalla stabilizzazione dei tassi d'interesse BCE/Fed e da inflazione core attorno al target del 2%, la strategia di allocazione privilegia strumenti con elevata efficienza fiscale:
...
3. **Pianificazione Successoria (D.Lgs. 346/1990):** Mantenimento della componente Titoli di Stato ed esenzioni legali per azzerare l'imposta sulle successioni."""
    ...
    if cash_drag:
        recommendations.insert(
            0,
            f"Ridurre il Cash Drag allocando € {rebalance.get('excess_cash_eur', 0.0):,.2f} su ETF monetari o governativi a breve termine.",
        )
...
def compute_family_office_multi_entity_consolidation(
    engine: Engine, portfolio_id: int = 1, custom_entities: Optional[List[Dict[str, Any]]] = None
) -> Dict[str, Any]:
    """
    ...
    if not custom_entities:
        ...
        custom_entities = [
            ...
            },
            {
                ...
                "gross_assets_eur": round(
                    nw.financial_investments * 0.6 + re_equity["total_property_market_value"] * 0.5, 2
                ),
    ...
    if not df_ent.empty and consolidated_family_office_net_worth > 0:
        df_ent["weight_on_consolidated_pct"] = (
            df_ent["consolidated_net_equity_eur"] / consolidated_family_office_net_worth
        ) * 100.0
    else:
...
def compute_sequence_of_returns_risk_engine(
    ...
    swr_safety_level = (
        "Conservativo (< 3.5%)"
        if swr_pct <= 3.5
        else ("Standard (3.5% - 4.2%)" if swr_pct <= 4.2 else "Rischioso (> 4.2%)")
    )
    ...
    return {
        ...
        "key_takeaways": [
            ...
            f"Un bear market iniziale senza protezione porta il capitale a zero all'anno {sim_early['ruin_year']}."
            if sim_early["is_ruined"]
            else "Il piano resiste anche all'Early Bear Market grazie a un solido capitale di partenza.",
...
def compute_multi_currency_fx_hedging_engine(
    ...
    default_exp = {"EUR": tot_w * 0.65, "USD": tot_w * 0.25, "GBP": tot_w * 0.05, "CHF": tot_w * 0.05}
    ...
    return {
        ...
        "hedged_scenario_comparison": {
            ...
            "50%_Rule_Of_Thumb": {
                "annual_cost_eur": round(tot_foreign_eur * 0.5 * 0.0175, 2),
                "shock_drawdown_eur": round(shock_unhedged_loss * 0.5, 2),
            },
...
def compute_family_governance_and_patti_di_famiglia(
    ...
    staggered_plan = [
        {
            "anno": 2026,
            "importo_donato_eur": min(biz_val * 0.5, 1000000.0),
            "franchigia_usata_eur": min(biz_val * 0.5, 1000000.0),
            "imposta_donazione_eur": 0.0,
            "note": "Cessione iniziale nuda proprietà con riserva di usufrutto",
        },
        {
            "anno": 2028,
            "importo_donato_eur": max(0.0, biz_val - 1000000.0),
            "franchigia_usata_eur": max(0.0, biz_val - 1000000.0),
            "imposta_donazione_eur": max(0.0, (biz_val - 1000000.0) * 0.04),
            "note": "Completamento passaggio quote con consolidamento usufrutto",
        },
    ]
...
def compute_smart_cashflow_reconciliation(
    engine: Engine, portfolio_id: int = 1, transactions_list: Optional[List[Dict[str, Any]]] = None
) -> Dict[str, Any]:
    """
    ...
    if transactions_list is not None:
        ...
        txs = [
            {
                "date": "2026-03-01",
                "description": "Bonifico Stipendio Datore Lavoro",
                "amount": 3200.0,
                "category": "Stipendio",
            },
            {
                "date": "2026-03-05",
                "description": "Addebito Rata Mutuo Intesa Sanpaolo",
                "amount": -850.0,
                "category": "Casa",
            },
            {
                "date": "2026-03-06",
                "description": "Abbonamento Netflix Mensile",
                "amount": -17.99,
            ...
            {
                "date": "2026-03-12",
                "description": "Bonifico PAC ETF Fineco Bank",
                "amount": -500.0,
