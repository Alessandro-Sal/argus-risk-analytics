# Design System: Investment Risk BI Platform
**Project ID:** capstone-risk-bi

## 1. Visual Theme & Atmosphere
The **Investment Risk BI Platform** embodies an institutional, high-density financial terminal aesthetic inspired by modern quantitative trading terminals. Designed for quantitative financial analysis, the atmosphere is sophisticated, futuristic, and utilitarian. 
...
* **Luminous Data Accents:** High-contrast accent glows (e.g., Bloomberg Amber `#ff9900`, Cyberpunk Cyan `#00f3ff`) highlight primary metrics, interactive hover targets, and structural section headers.
...
* **Cyberpunk Neon Cyan (`#00f3ff`):** Theme alternative accent color for high-tech data visualization modes and intense data callouts.
* **Emerald Wealth Green (`#00c853`):** Theme alternative accent color tailored for private wealth management modes.
...
* **Executive Health Green (`#3fb950` / `rgba(63, 185, 80, 0.15)`):** Used for positive PnL returns, low portfolio drawdown, and excellent Sharpe Ratio badges.
* **Executive Caution Gold (`#d29922` / `rgba(210, 153, 34, 0.15)`):** Used for moderate volatility, balanced risk profiles, and warning indicators.
* **Executive Danger Red (`#f85149` / `rgba(248, 81, 73, 0.15)`):** Used for negative PnL, high Value at Risk (VaR), aggressive drawdown alerts, and red status badges.
...
* **Modal Windows & Glossary Overlays:**
  ...
  * **Shadow Depth:** Heavy multi-layered shadow (`box-shadow: 0 20px 40px rgba(0,0,0,0.8), 0 0 20px rgba(255, 153, 0, 0.1)`).
...
3. **`2_🕸️_Correlazione_e_Cluster.py`**: Matrice di Correlazione e K-Means Clustering sui titoli.
4. **`3_🔬_Modelli_Quantitativi.py`**: Frontiera Efficiente Markowitz, Simulatore Ribilanciamento Tattico, Monte Carlo (Cholesky), Diagnostica Gomito (WCSS).
5. **`4_📋_Posizioni_e_Dettagli.py`**: Posizioni attive, Costo di carico FIFO, Ribilanciatore Tattico con prezzi reali, Flusso e Calendario Dividendi per Azienda.
6. **`5_🏛️_Valutazione_Aziendale.py`**: Multipli Fondamentali P/E e metriche societarie.
...
Tutti i pulsanti di esportazione (**Report PDF Factsheet 2 Pagine**, **Workbook Excel Multi-Tab .xlsx**, **CSV per Power BI / Looker Studio**) sono raggruppati in un'unica **Glass Card** posizionata **esclusivamente in fondo alla pagina `1_📈_Dashboard_Generale.py`**, eliminando ogni duplicato nella barra superiore o nelle schede secondarie.
