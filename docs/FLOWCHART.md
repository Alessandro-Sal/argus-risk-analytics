...
flowchart TD
    ...
    subgraph Layer5 ["📊 5. PRESENTATION & DESKTOP REPORTING LAYER"]
        direction TB
        APP("💻 Streamlit App / Control Room (8 Pagine Live)"):::frontend
...
- **Streamlit App & Desktop Launcher (`desktop_launcher.py` & `app.py`)**: Dashboard ad 8 pagine interattive fruibile via browser o come **Applicazione Desktop Nativa Windows** (`pywebview` + Edge WebView2) con l'icona dell'**Occhio di Argus**, gestione del ciclo di vita dei processi ed avvio protetto `wait_for_server`.
- **Standalone Executable & Release Pipeline (`scripts/build_desktop_app.py` & `scripts/package_release.py`)**: Pacchetto eseguibile standalone `ARGUS.exe` e generatore dell'archivio distribuiscibile `ARGUS_Desktop_v5.0.zip`.
