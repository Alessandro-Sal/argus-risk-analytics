...
from core.ui_utils import inject_custom_css, section, metric_card, fmt_eur, fmt_pct, render_workflow_stepper, render_command_bar, render_validation_report, render_db_status_badge, glossary_modal
import datetime
...
render_db_status_badge(engine_sidebar)
st.divider()
...
section("① Carica la Sorgente Dati (CSV o Google Sheets)")

data_source = st.radio(
    "Sorgente Dati",
    options=["CSV Standard (Template)", "Degiro (Export Transazioni)", "Google Sheets Live Sync"],
    horizontal=True,
    help="Scegli 'Degiro' per Transazioni.csv oppure 'Google Sheets' per la sincronizzazione live automatica."
)
...
with col_upload:
    if data_source == "Google Sheets Live Sync":
        ...
        gs_tab = st.text_input("Nome del Foglio (Tab):", value="History B/S Stocks")
        ...
        if btn_gs_sync:
            with st.spinner("⏳ Connessione a Google Sheets ed esecuzione ETL in corso..."):
                try:
                    ...
                    success = run_daily_pipeline(gs_name, gs_tab)
                    if success:
                        st.success("✅ Sincronizzazione con Google Sheets completata con successo! Ricarica lo Storico Analisi per visualizzare i risultati.")
...
if uploaded_file:
    ...
    with st.expander("🔍 Ispeziona Anteprima Dati Validati", expanded=False):
        ...
        st.dataframe(df_display_renamed, use_container_width=True, height=220)
...
# ── SEZIONE: Diagnostica di Sistema & Multi-Tier Cache Shield ───────────
st.markdown('<div style="margin-top: 25px;"></div>', unsafe_allow_html=True)
with st.expander("🩺 Diagnostica di Sistema & Multi-Tier Cache Shield (Health-Check)", expanded=False):
    st.caption("Verifica in tempo reale l'integrità dei database, la latenza dei 26 motori computazionali e lo scudo anti-rate limit della cache yfinance.")
    
    glossary_modal("ℹ️ Guida alla Diagnostica di Sistema & Caching Shield", """
    <p><b>1. Multi-Tier Caching & Rate-Limit Shield:</b><br>
    Per evitare blocchi e rate-limiting (<i>HTTP 429 Too Many Requests</i>) da parte dei provider esterni, ARGUS adotta un'architettura a doppio livello di persistenza:<br>
    • <b>Tier 1 (RAM LRU):</b> Accesso sub-millisecondo in memoria ad alta velocità.<br>
    • <b>Tier 2 (SQLite su Disco):</b> Salvataggio persistente su <code>data/yfinance_cache.db</code> con Time-to-Live (TTL) di 24 ore ($86.400$ secondi) ed exponential backoff con jitter casuale.<br>
    • <b>Fallback Offline:</b> In caso di assenza di rete, serve automaticamente l'ultimo dato valido disponibile.</p>

    <p><b>2. Benchmark di Latenza dei 26 Motori Quantitativi:</b><br>
    Misura in tempo reale i tempi di esecuzione (in millisecondi) dei moduli ad alta intensità computazionale (HRP, Black-Scholes, Regime Switching, Merton Jump-Diffusion, Forensic Accounting e Technical Charting).</p>

    <p><b>3. Integrità Database & Determinismo Stocastico:</b><br>
    Verifica la stabilità della connessione MySQL / SQLite locale e certifica che i seed dei generatori pseudo-casuali (Numpy PRNG) siano riproducibili e conformi agli standard istituzionali.</p>
    """, button_label="💡 Come funziona la Diagnostica & lo Scudo Cache?")
    ...
    with col_d1:
        metric_card("Stato Piattaforma", diag_res["overall_status"], f"{diag_res['health_score']:.0f}% Efficienza", positive=True)
    with col_d2:
        metric_card("Latenza Diagnostica Totale", f"{diag_res['total_diagnostic_latency_ms']:.1f} ms", "Tempo di risposta rapido", positive=True)
    with col_d3:
        cache_st = diag_res["cache_metrics"]
        metric_card("Multi-Tier Cache Shield", f"{cache_st['l2_disk_entries']} Voci SQLite", f"{cache_st['payload_size_kb']} KB salvati", positive=True)

    st.markdown("##### ⚡ Benchmark di Latenza Motori Quantitativi")
    st.dataframe(diag_res["engine_benchmarks"], use_container_width=True, hide_index=True)

    col_db_stat, col_cache_stat = st.columns(2)
    with col_db_stat:
        st.markdown("##### 🗄️ Integrità Database & Persistenza")
        st.dataframe(diag_res["database_checks"], use_container_width=True, hide_index=True)

    with col_cache_stat:
        st.markdown("##### 🛡️ Parametri Cache Multi-Tier")
        st.markdown(f"""
        * **Stato Scudo Anti-429**: `{cache_st['status']}`
        * **Versione Shield**: `{cache_st['shield_version']}`
        * **Oggetti in RAM (Tier 1)**: `{cache_st['l1_memory_items']}`
