...
from core.ui_utils import inject_custom_css, fmt_pct, metric_card, glossary_modal, apply_plotly_theme, render_risk_heatmap, render_command_bar
...
with tab1:
    ...
    if risk_contrib:
        ...
        <p><b>1. Cos'è il Market Regime Switching?</b><br>
        Un classificatore quantitativo che mappa lo stato macroeconomico del mercato in 3 regimi stocastici discreti, analizzando l'interazione congiunta tra rendimento medio e volatilità rolling a 21 giorni lavorativi.</p>

        <p><b>2. I 3 Regimi Macroeconomici:</b><br>
        • 🟢 <b>Regime 1 (Bull Low-Vol):</b> Rendimenti positivi e volatilità contenuta. Fase di espansione e trend rialzista solido.<br>
        • 🟡 <b>Regime 2 (Range-Bound):</b> Mercato laterale, rotazione settoriale e volatilità moderata. Consigliato ribilanciamento su Quality e Value.<br>
        • 🔴 <b>Regime 3 (Crisis / Stress):</b> Forte drawdown, spike di volatilità e crollo dei corsi. Consigliata protezione con liquidità e Put Hedging.</p>

        <p><b>3. Probabilità Recente:</b><br>
        Calcola la distribuzione di frequenza empirica dello stato di mercato nell'ultimo mese di contrattazione per guidare l'allocazione tattica di portafoglio.</p>
        """, button_label="💡 Come funziona il Market Regime Switching?")
        ...
        if regime_res:
            ...
            with col_reg1:
                metric_card("Stato Macroeconomico", f"{cur_icon} Regime {cur_idx}", cur_name, positive=(cur_idx == 1))
            with col_reg2:
                metric_card("Probabilità Bull Low-Vol", f"{probs.get('Bull Low-Vol', 0):.1f}%", "Espansione & Trend Rialzista", positive=True)
            with col_reg3:
                metric_card("Probabilità Crisi / Shock", f"{probs.get('Crisis High-Vol', 0):.1f}%", "Drawdown & Stress Test", positive=probs.get('Crisis High-Vol', 0) < 15.0)
