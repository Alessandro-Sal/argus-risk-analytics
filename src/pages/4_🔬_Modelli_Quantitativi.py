...
st.markdown(f"""
<div style="background: linear-gradient(90deg, rgba(22, 27, 34, 0.95) 0%, rgba(13, 17, 23, 0.85) 100%); border: 1px solid rgba(255,255,255,0.08); border-left: 4px solid {active_info['badge_color']}; border-radius: 8px; padding: 12px 18px; margin-top: 10px; margin-bottom: 22px;">
        ...
        if tickers_in_opt and not pos.empty:
            ...
            if sbx_mode == "🔢 Variazione Quote (Vendi / Compra N Azioni)":
                ...
                for _, r in edited_df_w.iterrows():
                    tk = str(r["ticker"])
                    sw = float(r["sim_weight_pct"])
                    st.session_state[f"sbx_w_{tk}"] = sw
                    sim_weights[tk] = sw
                    tot_raw_w += sw
