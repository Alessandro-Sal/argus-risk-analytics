...
from core.ui_utils import inject_custom_css, metric_card, glossary_modal, fmt_pct, render_executive_badges, render_formula_popover, apply_plotly_theme, render_command_bar
...
with tab4:
    ...
    with subtab_single:
        ...
        <p><b>1. Cos'è il Beneish M-Score (1999)?</b><br>
        È un modello econometrico forense a 8 fattori sviluppato dal Prof. Messod Beneish per determinare se una società quotata stia manipolando i propri bilanci o gonfiando artificialmente gli utili.<br>
        • <b>Soglia Critica:</b> $M > -1.78$ indica un'elevata probabilità di manipolazione contabile / frode (es. profilo Enron).<br>
        • <b>Soglia di Sicurezza:</b> $M \\le -1.78$ indica bilanci genuini, trasparenti e conformi.</p>

        <p><b>2. Gli 8 Indici Econometrici:</b><br>
        • <b>DSRI (Days Sales in Receivables):</b> Crescita anomala dei crediti commerciali rispetto al fatturato.<br>
        • <b>GMI (Gross Margin Index):</b> Deterioramento della marginalità lorda aziendale.<br>
        • <b>AQI (Asset Quality Index):</b> Capitalizzazione eccessiva di costi intangibili non operativi.<br>
        • <b>SGI (Sales Growth Index):</b> Tasso di espansione accelerato del fatturato.<br>
        • <b>DEPI (Depreciation Index):</b> Rallentamento artificioso degli ammortamenti.<br>
        • <b>SGAI (Sales General & Admin):</b> Aumento dei costi amministrativi e di vendita.<br>
        • <b>LVGI (Leverage Index):</b> Incremento dell'indebitamento strutturale totale.<br>
        • <b>TATA (Total Accruals to Total Assets):</b> Discrepanza tra utile contabile e flusso di cassa reale.</p>

        <p><b>3. Sloan Accrual Ratio (1996):</b><br>
        Misura la qualità degli utili rapportando l'utile netto al flusso di cassa operativo: $\\text{Accrual Ratio} = \\frac{\\text{Net Income} - \\text{Operating Cash Flow}}{\\text{Total Assets}}$. Valori inferiori a zero indicano flussi di cassa operativi eccellenti che superano l'utile di bilancio.</p>
        """, button_label="💡 Come funziona la Contabilità Forense (Beneish & Sloan)?")
