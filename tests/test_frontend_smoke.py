...
def test_frontend_pages_execution():
    ...
    # Verifichiamo la presenza di tutte le pagine Streamlit (almeno 7)
    page_files = glob.glob("src/pages/*.py") or glob.glob("pages/*.py")
    assert len(page_files) >= 7, f"Ci si aspettano almeno 7 pagine nella cartella pages/, trovate: {len(page_files)}"
